...
`source_pair` fields: version, symbol/session_date, currency (`USD`), source adapter identity, per-stream path/SHA-256/bytes/rows/schema hash, stream source/provenance identity, evidence references, retrieval-completeness status, and `SourceUnits` declaration. Preserve original units, conversion multiplier and its evidence. Bind quantity representation/precedence and coverage evidence to the exact object/lineage. A source method may be REST or another verified source method; method name alone is neither admission nor rejection. Version 1 descriptors retain verified terminal completion. Version 2 is reserved here for the accepted historical-corpus exception and must record `terminal_complete=false` plus `retrieval_completeness=unverified_missing_original_vendor_pagination_receipts`; it must never be rewritten as verified.

`member_context` fields: version/member, output coverage (`session_start_ns`, `end_ns`, `expected_rows`, `kind=full|prefix`), per-stream declared calculation intervals and evidence, precise gap intervals/instantaneous break events, halt intervals with stable IDs, unit/evidence references, and selection/discovery provenance. Each historical overlay carries information basis and known-at timestamp when available. Empty halt/break lists must say whether evidence establishes absence or the overlay is merely unavailable. Preserve unavailable knowledge metadata; do not assert measured live delivery. Nominal discovery/receipt mode does not gate measurement construction.

Reject unbounded/untrustworthy timestamp or member identity, conflicting duplicate source keys, unresolved quote units, unsupported required numeric representations, and unsupported requested calculation intervals before member completion. A member lacking the canonical files for its declared interval can appear in the admission report but cannot become an accepted complete base with invented observed zeros. Do not label assumed-empty overlays as verified historical halt absence. Report such a limitation independently of mechanical file completeness.
...
| Historical retrieval completeness | For the existing migrated historical T/Q corpus, the owner accepts that the original vendor-pagination receipts are unavailable. Calculation may use an identity-verified canonical file over its declared window, but every descriptor, admission report and base manifest records retrieval completeness as **unverified**. | Missing receipts alone are non-blocking for this historical corpus. Never fabricate a receipt, set `terminal_complete=true`, claim that empty pages were observed, or generalize this exception to a new acquisition. Missing/corrupt files, identity mismatch, schema failure, out-of-order SIP keys, precision failure, unresolved units, or events outside the declared calculation interval still block. |
...
This is a narrow, explicit historical-source policy, not a permissive production mode. Halt, unit, file-integrity, schema, ordering, precision and numerical-validity evidence remain mandatory. The exception is versioned in descriptors and outputs so downstream work cannot confuse transport/file correctness with verified vendor retrieval completeness.
...
Before decoding: freeze local sample manifest/identities, footer schemas, units/quantity representation evidence, the declared calculation interval, and available halt/continuity information. Record admissible and unresolved dependencies individually. For these retained historical objects, record missing original vendor-pagination receipts as `unverified_missing_original_vendor_pagination_receipts`; this accepted absence is not a reason to fabricate terminal-source evidence. If another dependency blocks the sample, produce that specific blocker; do not label the sample synthetic or substitute another member without a bounded revised scope.
