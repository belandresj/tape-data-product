...
The [September 11, 2026 acceptance record](offline.json) records these measurements, wheel SHA-256, implementation identity and runtime dependency versions. The audited wheel contains 79 package files matching source and no private implementation material. The full suite and final subset overlap; their counts must not be added as independent tests.

All measurements sample worker-plus-descendant RSS every 50 ms with a 768 MiB synthetic-work stop. No memory stop occurred. Tiny transaction fixtures explicitly supply fake disk capacity when testing unrelated behavior; separate tests verify production disk-reserve rejection.
...
Negative tests exercise reference-denominator completeness, earliest discovery, source mutation and discovery binding. Generated figures were visually inspected; numerical/export checks cover ties, zeros, eligibility, tails, normalized mass and count reconciliation. Published assets and original feature/query configuration hashes remain historical evidence.

## Repeat bounded checks

Follow [installation](../../../docs/dataset-build.md), then use fresh output paths:
...
No representative external sample or full external-data acceptance was executed. Exact historical accepted inputs were not supplied to this workflow. No full-run time/transfer/disk estimate is inferred from synthetic measurements.

A full external run requires a measured session-start sample on the exact final production path, with rows, elapsed time, peak process-tree RSS, transfer/spill and resource projections, followed by confirmation. Predictive and executable expectancy were not established by this acceptance run.
