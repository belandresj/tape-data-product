...
from tape_snapshot_inventory import digest

VERSION = 'tape_snapshot_features_v1'


def identity():
    return dict(version=VERSION, upstream=V.contract_identity(),
                publisher_sha256=V.sha256(__file__), horizons_seconds=[60,300], cadence_seconds=1,
...
def enrich(row, primitive):
    ...
    for key in ('trade_count','dollars','quote_age','trade_age','raw_messages','changes',
                'price_changes','size_changes','quote_source_file_accepted','trade_source_file_accepted'):
        row[f'primitive_{key}'] = primitive[key]
    for h in V.HORIZONS:
        a, b = row[f'midpoint_movement_bps_per_30s_{h}s'], row[f'quoted_spread_bps_{h}s']
        ratio = a/b if math.isfinite(a) and math.isfinite(b) and b > 0 else None
        row[f'movement_to_spread_{h}s'] = ratio
        row[f'movement_to_spread_valid_{h}s'] = bool(ratio is not None and row[f'movement_support_valid_{h}s']
            and row[f'quoted_spread_valid_{h}s'] and not row['halt_interval_active'])
    return {k: None if isinstance(v,float) and not math.isfinite(v) else v for k,v in row.items()}


def schema():
    fields = list(V.output_schema())
    fields += [pa.field('interval_end',pa.timestamp('ns','UTC')), pa.field('market_data_endpoint_ns',pa.int64()),
               pa.field('live_reproducible',pa.bool_())]
    for key in ('trade_count','dollars','quote_age','trade_age','raw_messages','changes','price_changes','size_changes'):
    ...
    for h in V.HORIZONS:
        fields += [pa.field(f'movement_to_spread_{h}s',pa.float64()),pa.field(f'movement_to_spread_valid_{h}s',pa.bool_())]
    return pa.schema(fields)


def write(quotes, trades, day, symbol, output, provenance, *, halts=(), seconds=57600, batch_size=25000):
    output = Path(output)
    if output.exists():
    ...
    contract = identity()
    meta = dict(contract=contract, config_hash=digest(contract), provenance=provenance,
                sample=seconds != 57600, expected_rows=seconds)
    out_schema = schema().with_metadata({b'tape_snapshot':json.dumps(meta,sort_keys=True).encode()})
    ...
    try:
        with pq.ParquetWriter(temporary,out_schema,compression='zstd') as writer:
            for p in V.primitives(quotes,trades,day,symbol,halts=halts,seconds=seconds,batch_size=batch_size,stats=stats):
                ...
                buffer.append(row)
                if len(buffer) == 1024:
                    writer.write_table(pa.Table.from_pylist(buffer,schema=out_schema)); buffer.clear()
            if buffer:
        ...
        raise
    return dict(**meta,counts=dict(counts),raw_stats=dict(stats),elapsed_seconds=time.monotonic()-started,
                sha256=V.sha256(output),bytes=output.stat().st_size)
    ...
    for batch in pf.iter_batches(batch_size=1024,use_threads=False):
        ...
        for field in batch.schema:
            if pa.types.is_floating(field.type):
                if any(x is not None and not math.isfinite(x) for x in rows[field.name]):
                    raise ValueError('nonfinite persisted feature')
        for h in V.HORIZONS:
            for a,b,r in zip(rows[f'midpoint_movement_bps_per_30s_{h}s'],rows[f'quoted_spread_bps_{h}s'],rows[f'movement_to_spread_{h}s']):
                expected_ratio = a/b if a is not None and b is not None and b > 0 else None
                if r != expected_ratio:
...
def distributions(path):
    ...
    groups={}
    columns=['session_date','interval_end_ns','halt_interval_active','state_mature_300s',*fields]
    for batch in pq.ParquetFile(path).iter_batches(batch_size=1024,columns=columns,use_threads=False):
        ...
        sessions={'premarket':seconds<19800,'rth':(seconds>=19800)&(seconds<43200),'after_hours':seconds>=43200}
        halt=table['halt_interval_active'].to_numpy();mature=table['state_mature_300s'].to_numpy()
        for session,mask in sessions.items():
            if not mask.any():continue
            for field in fields:
                key=session+'|'+field
                item=groups.setdefault(key,dict(rows=0,nulls=0,zeros=0,negative=0,underflow=0,overflow=0,
                    positive_histogram=[0]*72,minimum=None,maximum=None,halt_rows=0,mature_300s_rows=0))
                values=table[field].to_numpy()[mask]
                finite=values[np.isfinite(values)]
                item['rows']+=int(mask.sum());item['halt_rows']+=int(halt[mask].sum());item['mature_300s_rows']+=int(mature[mask].sum())
                item['nulls']+=int(len(values)-len(finite))
                if not len(finite):continue
                lo=float(finite.min());hi=float(finite.max())
                item['minimum']=lo if item['minimum'] is None else min(lo,item['minimum'])
                item['maximum']=hi if item['maximum'] is None else max(hi,item['maximum'])
                item['zeros']+=int((finite==0).sum());item['negative']+=int((finite<0).sum())
                positive=finite[finite>0]
                item['underflow']+=int((positive<edges[0]).sum());item['overflow']+=int((positive>=edges[-1]).sum())
                counts=np.histogram(positive[(positive>=edges[0])&(positive<edges[-1])],bins=edges)[0]
                item['positive_histogram']=[a+int(b) for a,b in zip(item['positive_histogram'],counts)]
    return dict(version='tape_feature_distribution_v1',weighting='one count per published second; dependent observations',
                eligibility='all published rows; quality and maturity counts are separate, not economic gates',
                positive_bin_edges=edges.tolist(),groups=groups)
