...
sys.path.insert(0, str(PROJECT_ROOT / "src/02_preprocessing"))
import build_market_state as MARKET  # noqa: E402
...
def atomic_parquet(
    ...
    *,
    ...
    if path.exists() and not replace:
        raise FileExistsError(path)
    table = pa.Table.from_pandas(value, preserve_index=False) if isinstance(value, pd.DataFrame) else value
    temporary = _tmp(path)
...
def verify_acquisition_manifest(path: Path, *, allow_unverified: bool = False) -> dict[str, Any]:
    path = path.expanduser().resolve()
    ...
    if sidecar.is_file():
        ...
        return {"verification": "hash_sidecar", "manifest": str(sidecar), "manifest_sha256": sha256_file(sidecar)}
    if frozen_manifest.is_file():
        ...
        if expected != sha256_file(path):
            raise ValueError(f"frozen acquisition registry hash mismatch: {frozen_manifest}")
        return {"verification": "frozen_registry_manifest", "manifest": str(frozen_manifest), "manifest_sha256": sha256_file(frozen_manifest)}
    ...
    return {"verification": "explicitly_unverified", "manifest": None, "manifest_sha256": None}
...
def load_symbol_day_manifest(
    ...
    *,
    ...
    if frame.duplicated(["session_date", "symbol"]).any():
        raise ValueError("symbol-day manifest has duplicate (session_date, symbol) rows")
    ...
    return frame.sort_values(["session_date", "symbol"], kind="mergesort").reset_index(drop=True)
...
def _duplicate_payload(table: pa.Table, row: int) -> tuple[Any, ...]:
    return tuple(
        _condition_payload(table[name][row].as_py()) if name == "conditions" else table[name][row].as_py()
...
def _window_values(times: np.ndarray, values: np.ndarray, start_ns: int, end_ns: int) -> np.ndarray:
...
def _pre_gap_qualified(predicate: np.ndarray, halt_start_index: int, config: DetectionConfig) -> bool:
...
def scan_trade_symbol_day(
    *,
    ...
    concat_i64 = lambda parts: np.concatenate(parts) if parts else np.empty(0, dtype=np.int64)
    concat_f64 = lambda parts: np.concatenate(parts) if parts else np.empty(0, dtype=np.float64)
    ...
    for index in np.flatnonzero(np.diff(eligible_times) >= config.gap_floor_seconds * NS):
        ...
        pre60_count = int(np.searchsorted(eligible_times, halt_start, side="left") - np.searchsorted(eligible_times, halt_start - 60 * NS, side="left"))
        pre300_count = int(np.searchsorted(eligible_times, halt_start, side="left") - np.searchsorted(eligible_times, halt_start - 300 * NS, side="left"))
        pre60_dollars = _prefix_interval(eligible_times, dollar_prefix, halt_start - 60 * NS, halt_start)
        pre300_dollars = _prefix_interval(eligible_times, dollar_prefix, halt_start - 300 * NS, halt_start)
        distances = [abs(gap_seconds - 300 * k) for k in (1, 2, 3, 4)]
        nearest_position = int(np.argmin(distances))

        raw_pre60 = _window_values(raw_price_times, raw_prices, halt_start - 60 * NS, halt_start)
        raw_pre300 = _window_values(raw_price_times, raw_prices, halt_start - 300 * NS, halt_start)
        raw_post60 = _window_values(raw_price_times, raw_prices, first_post, min(first_post + 60 * NS, end_ns))
        raw_post300 = _window_values(raw_price_times, raw_prices, first_post, min(first_post + 300 * NS, end_ns))
        eligible_pre60 = _window_values(eligible_times, eligible_prices, halt_start - 60 * NS, halt_start)
        eligible_pre300 = _window_values(eligible_times, eligible_prices, halt_start - 300 * NS, halt_start)
        eligible_post60 = _window_values(eligible_times, eligible_prices, first_post, min(first_post + 60 * NS, end_ns))
        eligible_post300 = _window_values(eligible_times, eligible_prices, first_post, min(first_post + 300 * NS, end_ns))
        review_start = max(start_ns, halt_start - 300 * NS)
        review_end = min(end_ns, first_post + 300 * NS)
        review_raw_left = int(np.searchsorted(raw_price_times, review_start, side="left"))
        review_raw_right = int(np.searchsorted(raw_price_times, review_end, side="left"))
        review_eligible_left = int(np.searchsorted(eligible_times, review_start, side="left"))
        review_eligible_right = int(np.searchsorted(eligible_times, review_end, side="left"))
        ...
        row: dict[str, Any] = {
            ...
            "reopening_jump_bps": float(10_000.0 * math.log(eligible_prices[index + 1] / eligible_prices[index])),
            "review_raw_trade_sip": raw_price_times[review_raw_left:review_raw_right].tolist(),
            "review_raw_trade_prices": raw_prices[review_raw_left:review_raw_right].tolist(),
            "review_eligible_trade_sip": eligible_times[review_eligible_left:review_eligible_right].tolist(),
            "review_eligible_trade_prices": eligible_prices[review_eligible_left:review_eligible_right].tolist(),
...
def _upstream_rejection_reason(job: Mapping[str, Any]) -> str | None:
    for field in (
        ...
        if field in job and not pd.isna(job[field]):
            value = job[field]
            accepted = (
                str(value).strip().lower() in {"true", "1", "yes", "accepted", "verified"}
...
def _write_symbol_day_checkpoint(
    *,
    ...
    if candidates:
        atomic_parquet(pd.DataFrame([dict(row) for row in candidates]), candidate_path, replace=False)
    else:
        atomic_parquet(_empty_candidate_table(), candidate_path, replace=False)
    atomic_parquet(
        pd.DataFrame(
            [dict(row) for row in presence],
            columns=["session_date", "symbol", "second_index", "first_eligible_sip", "last_eligible_sip"],
        ),
        presence_path,
        replace=False,
    )
...
def _load_symbol_day_checkpoint(
    *,
    ...
    if complete.get("trade_source_stat") != _source_stat(Path(job["trade_path"])):
        raise ValueError(f"trade source changed since checkpoint: {job['session_date']} {job['symbol']}")
    if complete.get("quote_source_stat") != _source_stat(Path(job["quote_path"])):
        raise ValueError(f"quote source changed since checkpoint: {job['session_date']} {job['symbol']}")
    if not load_data:
        return {"__checkpoint_path": str(target)}, [], []
    summary_rows = pd.read_parquet(target / "summary.parquet").to_dict("records")
    if len(summary_rows) != 1:
        raise ValueError(f"symbol-day checkpoint must contain one summary row: {target}")
    candidate_frame = pd.read_parquet(target / "candidates.parquet")
    presence_frame = pd.read_parquet(target / "presence.parquet")
    return summary_rows[0], candidate_frame.to_dict("records"), presence_frame.to_dict("records")
...
def detect_candidates(
    *,
    ...
    if run_dir.exists():
        if not resume:
            raise FileExistsError(f"detection run already exists; pass --resume only for an identical run: {run_dir}")
        if not config_path.is_file() or json.loads(config_path.read_text()) != run_config:
            raise ValueError("resume refused: run configuration or source manifest identity differs")
        ...
        if exact.is_file() and freeze.is_file():
            ...
            if record.get("exact_candidates_sha256") != sha256_file(exact):
                raise ValueError("resume refused: frozen candidate artifact hash mismatch")
            ...
            for row in summaries[summaries.status.eq("processed")].to_dict("records"):
                for stream in ("trade", "quote"):
                    ...
                    if (
                        ...
                        raise ValueError(f"resume refused: {stream} source identity changed for {row['session_date']} {row['symbol']}")
            return {
    ...
    for job in jobs.itertuples(index=False):
        ...
        try:
            ...
            job_candidates = []
            job_presence = []
        checkpoint_dirs.append(_write_symbol_day_checkpoint(
    ...
    _consolidate_checkpoints(checkpoint_dirs, filename="summary.parquet", output_path=paths["summaries"])
    ...
    _consolidate_checkpoints(checkpoint_dirs, filename="presence.parquet", output_path=paths["presence"], empty_table=pa.table({
        "session_date": pa.array([], pa.string()),
        "symbol": pa.array([], pa.string()),
    ...
    candidate_ids = pd.read_parquet(paths["candidates"], columns=["candidate_id"])["candidate_id"]
...
def _quote_evidence_for_candidate(
    ...
    quote_resume = int(sip[quote_resume_position]) if quote_resume_position < sip.size else None
    ...
    return {
        ...
        "valid_quote_message_count_in_gap": int(np.count_nonzero(in_gap & state.price_state_valid)),
        "invalid_or_one_sided_transition_count": int(np.count_nonzero(invalid_transition)),
        ...
        "last_pre_gap_valid_midpoint": float(state.midpoint[pre_valid[-1]]) if pre_valid.size else None,
        "first_post_gap_valid_midpoint": float(state.midpoint[post_valid[0]]) if post_valid.size else None,
        "last_pre_gap_spread_bps": float(state.spread_bps[pre_valid[-1]]) if pre_valid.size and np.isfinite(state.spread_bps[pre_valid[-1]]) else None,
        "first_post_gap_spread_bps": float(state.spread_bps[post_valid[0]]) if post_valid.size and np.isfinite(state.spread_bps[post_valid[0]]) else None,
        ...
        "review_quote_midpoint": [
            float(value) if np.isfinite(value) else None for value in state.midpoint[review]
        ],
...
def _witness_count(
    ...
    *,
    ...
    start, stop = bounds
    in_gap = (
        (presence_first[start:stop] < first_post)
...
def enrich_candidates(
    *,
    ...
    if not freeze_path.is_file() or not candidate_path.is_file():
        raise FileNotFoundError("detect stage and candidate freeze are required before enrichment")
    ...
    for (day, symbol), group in candidates.groupby(["session_date", "symbol"], sort=True):
        first = group.iloc[0]
        quote_path = Path(str(first.quote_source_path))
        identity = parquet_identity(quote_path)
        if identity["sha256"] != first.quote_source_sha256 or identity["parquet_metadata_sha256"] != first.quote_source_parquet_metadata_sha256:
            raise ValueError(f"quote source identity changed before enrichment: {quote_path}")
    ...
    merged = candidates.merge(evidence, on=["candidate_id", "session_date", "symbol"], validate="one_to_one")
    rejected_by_date = (
    ...
    merged = merged.merge(rejected_by_date, on="session_date", how="left", validate="many_to_one")
    ...
    for row in merged.to_dict("records"):
        ...
        assert last_pre is not None and first_post is not None
        count = 0
        for other in merged[merged.session_date == row["session_date"]].to_dict("records"):
            if other["symbol"] == row["symbol"] or int(other["quote_message_count_in_gap"]) != 0:
                continue
            ...
            if _overlap_fraction(last_pre, first_post, other_start, other_end) >= config.overlapping_gap_fraction:
                count += 1
        overlap_counts.append(count)
    merged["overlapping_candidate_symbol_count"] = overlap_counts
    merged["source_health_class"] = [
        classify_source_health(
            same_symbol_quote_stream_observed_in_gap=bool(row["same_symbol_quote_stream_observed_in_gap"]),
            other_symbol_event_witness_count=int(row["other_symbol_event_witness_count"]),
            same_date_upstream_rejected_symbol_count=int(row["same_date_upstream_rejected_symbol_count"]),
            acquired_symbol_count=int(row["acquired_symbol_count"]),
            overlapping_candidate_symbol_count=int(row["overlapping_candidate_symbol_count"]),
            config=config,
        )
        for row in merged.to_dict("records")
    ]
    evidence_columns = [column for column in merged.columns if column not in candidates.columns or column in {"candidate_id", "session_date", "symbol"}]
...
EXTERNAL_ALIASES = {
    ...
    "official_halt_start": ("official_halt_start", "halt_start", "halt_time", "halt_timestamp"),
    "official_quote_resume_time": ("official_quote_resume_time", "quote_resume_time", "quote_resume"),
    "official_trade_resume_time": ("official_trade_resume_time", "trade_resume_time", "resume_time", "trade_resume"),
...
def normalize_external_halts(
    *,
    ...
    if metadata_path.exists():
        ...
        if existing.get("raw_source_sha256") != raw_hash or existing.get("provider") != provider:
    ...
    for position, source_row in enumerate(raw.to_dict("records")):
        ...
        start = _official_timestamp(_alias_value(source_row, "official_halt_start"), day)
        resume_day_value = _alias_value(source_row, "official_resume_date")
        resume_day = day if resume_day_value is None else pd.Timestamp(resume_day_value).date().isoformat()
        quote_resume = _official_timestamp(_alias_value(source_row, "official_quote_resume_time"), resume_day)
        trade_resume = _official_timestamp(_alias_value(source_row, "official_trade_resume_time"), resume_day)
            ...
            event_id = canonical_hash(
                {
                    ...
                    "session_date": day,
                    "symbol": symbol,
                    "official_halt_start": None if pd.isna(start) else start.isoformat(),
                    "official_trade_resume_time": None if pd.isna(trade_resume) else trade_resume.isoformat(),
                    "raw_row": position,
                }
            )
        rows.append(
    ...
    if not normalized.empty:
        if normalized.duplicated(["provider", "provider_event_id"]).any():
            raise ValueError("normalized external records contain duplicate provider event IDs")
...
def _match_score(candidate: Mapping[str, Any], official: Mapping[str, Any], config: DetectionConfig) -> tuple[int, str] | None:
    ...
    if official_end is not None:
        overlap = max(0, min(candidate_end, official_end) - max(candidate_start, official_start))
        return (overlap, "interval_overlap") if overlap > 0 else None
    if abs(candidate_start - official_start) <= config.start_only_match_tolerance_seconds * NS:
...
def match_external_one_to_one(
    ...
    for day, symbol in keys:
        crows = candidates[(candidates.session_date.astype(str) == day) & (candidates.symbol.astype(str) == symbol)].to_dict("records")
        orows = official[(official.session_date.astype(str) == day) & (official.symbol.astype(str) == symbol)].to_dict("records")
        ...
        if len(orows) <= 20:
            ...
            def solve(i: int, used: int) -> tuple[int, tuple[tuple[int, int], ...]]:
                ...
                for j in range(len(orows)):
                    ...
                    if total > best_score or (total == best_score and proposal < best_pairs):
            ...
            used_i: set[int] = set()
            used_j: set[int] = set()
            for (i, j), (score, _) in sorted(scores.items(), key=lambda item: (-item[1][0], item[0])):
            ...
            output.append(
                {
                    ...
                    "positive_overlap_seconds": score / NS if match_type == "interval_overlap" else None,
                }
            )
    return pd.DataFrame(
        output,
        columns=["candidate_id", "provider", "provider_event_id", "session_date", "symbol", "match_type", "positive_overlap_seconds"],
    )


def _activity_before_official(official_row: Mapping[str, Any], summaries: pd.DataFrame, config: DetectionConfig) -> bool:
    match = summaries[
    ...
    return bool(np.any((values <= start) & (values >= start - config.pre_gap_grace_seconds * NS)))


def _official_relevance_reason(row: Mapping[str, Any], summaries: pd.DataFrame, config: DetectionConfig) -> tuple[bool, str]:
...
def deterministic_review_sample(frame: pd.DataFrame, config: DetectionConfig) -> pd.DataFrame:
    if frame.empty:
        return frame.copy()
    sample = frame.copy()
    sample["duration_band"] = np.where(
        sample.near_300s_multiple_90s,
        sample.nearest_300s_multiple.map({1: "near_5m", 2: "near_10m", 3: "near_15m", 4: "near_20m"}),
        "non_multiple",
    )
    sample["session_segment"] = sample.inferred_halt_interval_start.map(_session_segment)
    median_volatility = pd.to_numeric(sample.pre_gap_range_bps_300s, errors="coerce").median()
    sample["pre_gap_volatility_band"] = np.where(
        pd.to_numeric(sample.pre_gap_range_bps_300s, errors="coerce") >= median_volatility,
        "high",
        "low",
    )
    sample["quote_activity_band"] = np.where(sample.quote_message_count_in_gap > 0, "quote_active", "quote_silent")
    sample["review_rank_hash"] = sample.candidate_id.map(
        lambda value: canonical_hash({"seed": config.manual_review_seed, "candidate_id": value})
    )
    strata = ["duration_band", "session_segment", "instrument_type", "pre_gap_volatility_band", "quote_activity_band"]
    ...
    return sample.head(min(config.manual_review_target, len(sample))).reset_index(drop=True)
...
def _render_review_plot(row: Mapping[str, Any], path: Path) -> None:
    ...
    eligible_times = pd.to_datetime(row.get("review_eligible_trade_sip", []), unit="ns", utc=True)
    eligible_prices = np.asarray(row.get("review_eligible_trade_prices", []), dtype=float)
    quote_times = pd.to_datetime(row.get("review_quote_sip", []), unit="ns", utc=True)
    quote_mid = np.asarray([np.nan if value is None else value for value in row.get("review_quote_midpoint", [])], dtype=float)
    start = pd.Timestamp(row["inferred_halt_interval_start"])
    resume = pd.Timestamp(row["inferred_trade_resume_time"])
    figure, axis = plt.subplots(figsize=(10, 4))
    if len(raw_times):
        axis.scatter(raw_times, raw_prices, s=5, alpha=0.25, label="raw finite-price trades")
    if len(eligible_times):
        axis.scatter(eligible_times, eligible_prices, s=8, alpha=0.8, label="eligible trades")
    if len(quote_times):
        axis.plot(quote_times, quote_mid, linewidth=0.8, alpha=0.7, label="valid quote midpoint")
    axis.axvspan(start, resume, color="tab:red", alpha=0.14, label="inferred interruption")
    axis.set_title(f"{row['session_date']} {row['symbol']} — raw T/Q interruption evidence")
    ...
    return canonical_hash(
        {
            ...
            "trade_resume_time": None if pd.isna(row.get("trade_resume_time")) else pd.Timestamp(row["trade_resume_time"]).isoformat(),
...
def _candidate_registry(
    ...
    combined = candidates.merge(evidence, on=["candidate_id", "session_date", "symbol"], how="left", validate="one_to_one")
    matched = matches.merge(
        official,
        on=["provider", "provider_event_id", "session_date", "symbol"],
        how="left",
        validate="one_to_one",
    ) if not matches.empty else matches.copy()
    ...
    for candidate in combined.to_dict("records"):
        ...
        if external is not None:
            ...
            if official_start is not None and official_resume is not None:
                ...
                candidate["effective_interval_source"] = "official_halt_and_trade_resume"
            elif official_start is not None:
                candidate["halt_interval_start"] = _utc(official_start)
                candidate["trade_resume_time"] = candidate["inferred_trade_resume_time"]
                candidate["effective_interval_source"] = "official_start_inferred_trade_resume"
            else:
            ...
            if pre and post and healthy:
                ...
                candidate["accepted_for_experiment"] = True
                candidate["acceptance_reason"] = "active_gap_resumption_with_healthy_source_witness"
            elif pre and (post ^ healthy):
                candidate["acceptance_class"] = "probable_trading_interruption"
                candidate["accepted_for_experiment"] = False
                candidate["acceptance_reason"] = "exactly_one_of_post_activity_or_source_health_unresolved"
            else:
    ...
    matched_external_ids = set(zip(matched.provider, matched.provider_event_id)) if not matched.empty else set()
    processed_keys = set(
        zip(
    ...
    for external in official.to_dict("records"):
        if (external["provider"], external["provider_event_id"]) in matched_external_ids:
            continue
        ...
        usable = key in processed_keys and start is not None and resume is not None and resume > start
        ...
        row = {
            ...
            "effective_interval_source": "official_halt_and_trade_resume" if usable else "unusable_external_bounds",
            "acceptance_class": "externally_confirmed" if usable else "ambiguous_symbol_gap",
            "accepted_for_experiment": bool(usable),
            "acceptance_reason": "unmatched_official_positive_with_usable_bounds" if usable else "external_positive_lacks_usable_bounds_or_raw_tq_qa",
            ...
            "source_provenance": canonical_json(
                {
                    "external": {"provider": external.get("provider"), "event_id": external.get("provider_event_id"), "sha256": external.get("raw_source_sha256")},
                    "trade": {"path": source.get("trade_source_path"), "sha256": source.get("trade_source_sha256"), "parquet_metadata_sha256": source.get("trade_source_parquet_metadata_sha256")},
                    "quote": {"path": source.get("quote_source_path"), "sha256": source.get("quote_source_sha256"), "parquet_metadata_sha256": source.get("quote_source_parquet_metadata_sha256")},
                }
            ),
    ...
    if not registry.empty:
        registry["halt_interval_id"] = [_registry_interval_id(row) for row in registry.to_dict("records")]
...
def validate_detection_run(
    *,
    ...
    if candidate_freeze.get("exact_candidates_sha256") != sha256_file(candidate_path):
        raise ValueError("blind candidate freeze hash no longer matches exact_candidates.parquet")
    run_config = json.loads((run_dir / "run_config.json").read_text())
    if run_config.get("candidate_config_hash") != config.digest():
        raise ValueError("validation configuration differs from frozen detection configuration")
    ...
    if not matches.empty:
        candidate_bounds = candidates[
            ["candidate_id", "inferred_halt_interval_start", "inferred_trade_resume_time"]
        ]
        ...
        matches = matches.merge(candidate_bounds, on="candidate_id", validate="one_to_one")
        ...
        matches["candidate_to_official_overlap_fraction"] = (
            pd.to_numeric(matches.positive_overlap_seconds, errors="coerce") / candidate_duration
        )
    ...
    matched_external = set(zip(matches.provider, matches.provider_event_id)) if not matches.empty else set()
    ...
    for row in official.to_dict("records"):
        ...
        if not matched:
            if not relevant:
                ...
                miss_reason = "frozen_candidate_interval_did_not_match_official_interval"
        enriched = dict(row)
        enriched.update(
            {
    ...
    if not official_eval.empty:
        ...
        official_eval["duration_band"] = pd.cut(
            ...
            labels=["up_to_6_5m", "6_5_to_11_5m", "11_5_to_16_5m", "16_5_to_21_5m", "over_21_5m"],
        ).astype("string")
    unmatched_official = pd.DataFrame(unmatched_rows, columns=list(official_eval.columns))
    atomic_parquet(unmatched_official, run_dir / "unmatched_official_intervals.parquet")

    dates = sorted(official.session_date.astype(str).unique()) if not official.empty else []
    ...
    confirmation_relevant = official_eval[
        official_eval.get("experiment_relevant", pd.Series(dtype=bool)).fillna(False)
        & official_eval.get("detector_split", pd.Series(dtype=str)).eq("confirmation")
    ] if not official_eval.empty else official_eval
    recall_denominator = len(confirmation_relevant)
    recall_numerator = int(confirmation_relevant.detector_matched.sum()) if recall_denominator else 0
    interval_recall = recall_numerator / recall_denominator if recall_denominator else None
    relevant_symbol_days = confirmation_relevant[["session_date", "symbol"]].drop_duplicates() if recall_denominator else pd.DataFrame(columns=["session_date", "symbol"])
    matched_symbol_days = confirmation_relevant.loc[confirmation_relevant.detector_matched, ["session_date", "symbol"]].drop_duplicates() if recall_denominator else relevant_symbol_days
    symbol_day_recall = len(matched_symbol_days) / len(relevant_symbol_days) if len(relevant_symbol_days) else None

    inferred_pool = registry[
    ...
    if not review_manifest.empty:
        review_manifest["plot_path"] = [str(plots_root / f"{candidate_id}.png") for candidate_id in review_manifest.candidate_id]
        if render_plots:
    ...
    atomic_csv(review_manifest.reindex(columns=review_columns + ["plot_path"]), review_manifest_path)
    ...
    if review_path.exists():
        ...
        if set(review.candidate_id.astype(str)) != expected_ids:
            raise ValueError("manual_review.csv candidate IDs differ from the deterministic review manifest")
    else:
        review = review_manifest.reindex(columns=["candidate_id", "session_date", "symbol"]).copy()
    ...
    valid_source = {"yes", "no"}
    credible = review.credible_interruption.astype(str).str.lower() if "credible_interruption" in review else pd.Series(dtype=str)
    source_concern = review.source_concern.astype(str).str.lower() if "source_concern" in review else pd.Series(dtype=str)
    review_complete = (
        len(review) == len(review_manifest)
        and credible.isin(valid_credible).all()
        and source_concern.isin(valid_source).all()
        and (len(inferred_pool) < config.manual_review_target or len(review) >= config.manual_review_target)
    )
    credible_fraction = float(credible.eq("yes").mean()) if review_complete and len(review) else None
    no_reviewed_source_concern = bool(~source_concern.eq("yes").any()) if review_complete else False
    if not registry.empty and len(review):
        reviewed_yes = set(review.loc[credible.eq("yes"), "candidate_id"].astype(str))
        reviewed_unclear = set(review.loc[credible.eq("unclear"), "candidate_id"].astype(str))
        selected = set(review.candidate_id.astype(str))
        registry.loc[registry.candidate_id.astype(str).isin(selected), "manual_review_status"] = "reviewed_no"
        registry.loc[registry.candidate_id.astype(str).isin(reviewed_yes), "manual_review_status"] = "reviewed_credible"
        registry.loc[registry.candidate_id.astype(str).isin(reviewed_unclear), "manual_review_status"] = "reviewed_unclear"

    source_rows_accepted = bool(
        ((registry.acceptance_class == "possible_source_interruption") & registry.accepted_for_experiment).any()
    ) if not registry.empty else False
    relevant_misses = confirmation_relevant[~confirmation_relevant.detector_matched] if recall_denominator else confirmation_relevant
    misses_reported = bool(relevant_misses.failure_reason.fillna("").astype(str).str.len().gt(0).all()) if len(relevant_misses) else True
    gate_checks = {
        "confirmation_recall_at_least_95_percent": interval_recall is not None and interval_recall >= 0.95,
        ...
        "manual_credible_fraction_at_least_90_percent": credible_fraction is not None and credible_fraction >= 0.90,
        "manual_review_has_no_source_concern": no_reviewed_source_concern,
        "configuration_unchanged_since_candidate_freeze": candidate_freeze.get("candidate_config_hash") == config.digest(),
    }
    validation_gate_passed = all(gate_checks.values())

    if not registry.empty:
        registry = registry.sort_values(
            ["session_date", "symbol", "halt_interval_start", "trade_resume_time", "halt_interval_id"],
            kind="mergesort",
            na_position="last",
    ...
    class_rows = []
    if not registry.empty:
        for acceptance_class, frame in registry.groupby("acceptance_class", dropna=False):
            class_rows.append(
                {
    ...
    if not official_eval.empty:
        for dimension in ("official_market", "official_reason", "session_segment", "duration_band"):
                ...
                class_rows.append(
                    {
                        ...
                        "accepted_rows": int(relevant_frame.detector_matched.sum()) if len(relevant_frame) else 0,
                    }
                )
    atomic_csv(pd.DataFrame(class_rows, columns=["dimension", "value", "rows", "accepted_rows"]), run_dir / "validation_by_class.csv")
    ...
    atomic_json(
        {
            ...
            "status": "validation_complete" if review_complete else "manual_review_pending",
            "detection_method_version": DETECTION_METHOD_VERSION,
            "registry_version": REGISTRY_VERSION,
            "registry_config_hash": registry_config_hash,
            "content_sha256": {
...
def validate_registry_frame(registry: pd.DataFrame, acquired: pd.DataFrame) -> None:
    ...
    if registry.halt_interval_id.isna().any() or registry.halt_interval_id.duplicated().any():
    ...
    if not candidate_hashes.str.fullmatch(r"[0-9a-f]{64}").all() or candidate_hashes.nunique() > 1:
    ...
    acquired_keys = set(zip(acquired.session_date.astype(str), acquired.symbol.astype(str)))
    registry_keys = set(zip(registry.session_date.astype(str), registry.symbol.astype(str)))
    outside = registry_keys - acquired_keys
    if outside:
        raise ValueError(f"registry contains rows outside acquired manifest: {sorted(outside)[:5]}")
    ...
    for row in registry.to_dict("records"):
        ...
        effective_end = session_end if resume is None or resume > session_end else resume
        ...
        if acceptance_class == "inferred_high_confidence" and accepted:
            if not (
                ...
                raise ValueError("accepted inferred interval lacks mandatory activity/source evidence")
    ...
    for row in accepted_rows:
        if previous is not None and row[0:2] == previous[0:2] and row[2] < previous[3]:
            raise ValueError(f"accepted same-symbol intervals overlap: {previous[5]} and {row[5]}")
        previous = row
...
def verify_frozen_registry(registry_dir: Path) -> dict[str, Any]:
    ...
    if manifest.get("status") != "frozen" or manifest.get("registry_version") != REGISTRY_VERSION:
    ...
    if not pa.Table.from_pandas(expected, preserve_index=False).equals(
        ...
        raise ValueError("accepted interval projection differs from authoritative registry")
    return manifest
...
def freeze_registry(
    *,
    ...
    if output_dir.exists():
        raise FileExistsError(f"registry version directory is immutable and already exists: {output_dir}")
    ...
    if external_only:
        if not validation.get("external_only_publication_allowed"):
            raise ValueError("no usable authoritative external-only intervals are available")
    elif not validation.get("validation_gate_passed"):
        raise ValueError("inferred V1 registry freeze refused: required validation gate did not pass")
    registry = pd.read_parquet(draft_path)
    publication_mode = "external_only" if external_only else "validated_external_and_inferred"
    if external_only:
    ...
    try:
        ...
        definition = {
            ...
            "candidate_config": json.loads((run_dir / "run_config.json").read_text())["candidate_config"],
        ...
        content = {
            ...
            for path in (registry_path, accepted_path, summary_csv, definition_path, readme_path)
        }
        manifest = {
...
def _set_or_append(table: pa.Table, name: str, values: pa.Array | pa.ChunkedArray) -> pa.Table:
...
def halt_adjusted_activity_state(
    ...
    return {
        ...
        "halt_adjusted_activity_episode_number": pa.array(episode_number, type=pa.int32()),
        "halt_adjusted_activity_activation_time": pa.array(activation, type=pa.timestamp("ns", tz="UTC")),
        "halt_adjusted_activity_episode_origin": pa.array(origin, type=pa.timestamp("ns", tz="UTC")),
        "halt_adjusted_activity_episode_end": pa.array(episode_end, type=pa.timestamp("ns", tz="UTC")),
...
def apply_halt_overlay(
    ...
    *,
    ...
    ends = np.asarray(pc.cast(panel["interval_end"].combine_chunks(), pa.int64()).to_numpy(), dtype=np.int64)
    starts = ends - NS
    ...
    for interval in intervals.to_dict("records"):
        ...
        halt_end = session_end if resume_value is None or resume_value > session_end else resume_value
    ...
    output = panel
    output = _set_or_append(output, "halt_registry_applied", pa.array([True] * panel.num_rows))
    output = _set_or_append(output, "halt_registry_version", pa.array([registry_manifest["registry_version"]] * panel.num_rows))
    output = _set_or_append(output, "halt_registry_config_hash", pa.array([registry_manifest["registry_config_hash"]] * panel.num_rows))
    output = _set_or_append(output, "halt_interval_active", pa.array(active))
    output = _set_or_append(output, "halt_interval_id", pa.array(interval_ids, type=pa.string()))
    output = _set_or_append(output, "halt_overlay_end_reason", pa.array(close_reason, type=pa.string()))
    output = _set_or_append(output, "post_halt_insufficient_activity_warmup", pa.array(post_activity_warmup))
    output = _set_or_append(output, "post_halt_insufficient_reference_capacity", pa.array(post_reference_warmup))
    base_segment = (
        np.asarray(panel["continuity_segment_id"].combine_chunks().to_numpy(), dtype=np.int64)
    ...
    output = _set_or_append(output, "halt_adjusted_continuity_segment_id", pa.array(adjusted))

    if {"eligible_trade_count", "eligible_dollar_volume"} <= set(panel.column_names):
        activity_columns = halt_adjusted_activity_state(
            np.asarray(panel["eligible_trade_count"].combine_chunks().to_numpy(), dtype=np.int64),
            np.asarray(panel["eligible_dollar_volume"].combine_chunks().to_numpy(), dtype=np.float64),
            ends,
            active,
        )
    ...
    for name in episode_local_fields:
        ...
        masked = pc.if_else(pa.array(null_mask), pa.nulls(panel.num_rows, column.type), column)
        output = _set_or_append(output, name, masked)
    metadata = dict(output.schema.metadata or {})
    metadata.update(
        {
            b"halt_registry_applied": b"true",
            b"halt_registry_version": str(registry_manifest["registry_version"]).encode(),
            b"halt_registry_config_hash": str(registry_manifest["registry_config_hash"]).encode(),
            b"halt_registry_sha256": str(registry_manifest["content_sha256"]["registry.parquet"]).encode(),
            b"accepted_intervals_sha256": str(registry_manifest["content_sha256"]["accepted_intervals.parquet"]).encode(),
...
def interval_overlaps_any(start_ns: int, end_ns: int, intervals: pd.DataFrame, session_date: str, symbol: str) -> bool:
    if end_ns <= start_ns:
        raise ValueError("experiment interval is reversed or zero length")
    for row in _accepted_for_symbol_day(intervals, session_date, symbol).to_dict("records"):
        session_start, session_end = session_bounds_ns(session_date)
...
def add_experiment_overlap_flags(
    ...
    *,
    ...
    for row in result.to_dict("records"):
        ...
        flags["anchor_in_accepted_halt"].append(interval_overlaps_any(point, point + 1, registry, day, symbol))
        for output_name, left_name, right_name in (
            ("reference_overlaps_accepted_halt", reference_start, reference_end),
            ("diagnostic_window_overlaps_accepted_halt", diagnostic_start, diagnostic_end),
            ("future_outcome_overlaps_accepted_halt", future_start, future_end),
        ):
            left, right = _ns(row.get(left_name)), _ns(row.get(right_name))
            flags[output_name].append(False if left is None or right is None else interval_overlaps_any(left, right, registry, day, symbol))
    ...
    atomic_json(
        {
            ...
            "accepted_intervals_sha256": manifest["content_sha256"]["accepted_intervals.parquet"],
        },
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    detect = subparsers.add_parser("detect", help="stream raw trades and freeze exact T/Q candidates")
    ...
    detect.add_argument("--workers", type=int, choices=[1], default=1, help="V1 defaults to one bounded sequential worker")
    detect.add_argument("--checkpoint-root", type=Path)
    detect.add_argument("--checkpoint-read-root", type=Path, action="append", default=[])

    enrich = subparsers.add_parser("enrich", help="read quotes only for exact-candidate symbol-days")
    ...
    normalize = subparsers.add_parser("normalize-external", help="immutably cache and normalize official positives")
    ...
    validate = subparsers.add_parser("validate", help="blind-match, split, audit, and evaluate the V1 gate")
    validate.add_argument("--run-dir", required=True, type=Path)
    validate.add_argument("--no-review-plots", action="store_true")

    freeze = subparsers.add_parser("freeze", help="publish an immutable curated registry")
    ...
    overlay = subparsers.add_parser("overlay", help="apply a frozen registry to a historical second panel")
    ...
    verify = subparsers.add_parser("verify", help="rehash and verify an immutable curated registry")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "detect":
        ...
        print(f"Frozen {pq.ParquetFile(outputs['candidates']).metadata.num_rows} exact candidates at {outputs['candidates']}")
    elif args.command == "enrich":
        output = enrich_candidates(run_dir=args.run_dir, config=DetectionConfig(batch_size=args.batch_size))
        ...
        summary = validate_detection_run(run_dir=args.run_dir, render_plots=not args.no_review_plots)
        print(
            f"Validation gate passed={summary['validation_gate_passed']} "
            f"recall={summary['official_experiment_relevant_interval_recall']}"
        )
    elif args.command == "freeze":
        outputs = freeze_registry(run_dir=args.run_dir, output_dir=args.output_dir, external_only=args.external_only)
