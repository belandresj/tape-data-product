...
class Moments:
    def __init__(self, capacity):
        self.capacity=capacity; self.queue=deque(); self.count=self.positive=0
        self.s1=self.s2=0
    ...
    def append(self, value):
        if len(self.queue)==self.capacity:
            ...
            if old is not None:
                self.count-=1;self.positive-=old>0;self.s1-=old;self.s2-=old*old
        x=None
        if value is not None and math.isfinite(value):
            if value<0:raise ValueError('negative movement')
            x=self.units(float(value));self.count+=1;self.positive+=x>0
            self.s1+=x;self.s2+=x*x
        self.queue.append(x)
        if not self.positive:
            if self.s1 or self.s2:raise ValueError('nonzero moments with zero positive count')

    def mean(self):
        return self.s1/(self.count*(1<<1074)) if self.count else math.nan

    def participation(self):
        if not self.positive:return math.nan
        if self.s1<=0 or self.s2<=0:raise ValueError('invalid exact moments')
