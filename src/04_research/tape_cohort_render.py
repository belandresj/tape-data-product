...
def render(run):
    ...
    if manifest.get("state")!="complete": raise ValueError("cannot render incomplete run")
    summary=json.loads((root/"summary.json").read_text())
    text=("# Tape cohort supply summary\n\n"
    ...
    path=root/"supply_summary.md";path.write_text(text);return {"summary":str(path),"network_requests":0}
