...
def entry_from_record(record):
    ...
    if r.get('state') != 'complete' or type(r.get('rows_verified')) is not int or r['rows_verified'] != 57600:
    ...
    pub = r['publication']
    if pub.get('bucket') != 'massive-equities' or pub.get('manifest_key') != prefix + '/manifest.json':
        raise ValueError('invalid completion locator')
    objects = {name: object_identity(pub['objects'][name], prefix, name)
               for name in ('features', 'support', 'manifest')}
    return dict(session_date=day, symbol=symbol, expected_rows=57600,
                partition_identity=pid, bucket=pub['bucket'], objects=objects,
                receipt_sha256=record['receipt_sha256'], receipt_path=record['receipt_path'])


def freeze(output, *, run=None, capture=None, label='Completed-only convergence preview; not a six-month release'):
    """Exclude malformed publications with an explicit exclusion ledger."""
    output = Path(output); output.mkdir(parents=True, exist_ok=False)
    cutoff = time.time(); exclusions = []; counts = Counter(); entries = []; seen = set()
    def source():
        if capture:
        ...
        for path in sorted((Path(run) / 'receipts').glob('*.json')):
            try:
                raw = path.read_bytes()
                if len(raw) > MAX_RECORD: raise ValueError('oversized receipt')
                r = json.loads(raw); counts[r.get('state', 'missing_state')] += 1
                if r.get('state') != 'complete' or r.get('finished_at', cutoff+1) > cutoff:
                    continue
                payload = read_json(Path(run) / 'payloads' / path.name)
                yield dict(receipt=r, member=payload['member'], receipt_path=str(path.resolve()),
                           receipt_sha256=hashlib.sha256(raw).hexdigest())
            except (ValueError, KeyError, TypeError, OSError) as exc:
                exclusions.append(dict(path=str(path), reason=str(exc)))
    for record in source():
        try:
            e = entry_from_record(record); key = (e['session_date'], e['symbol'])
            if key in seen: raise ValueError('duplicate completed symbol-day')
            seen.add(key); entries.append(e)
        except (ValueError, KeyError, TypeError) as exc:
            exclusions.append(dict(path=record.get('receipt_path'), reason=str(exc)))
    if not entries: raise ValueError('no valid completed publications')
    ...
    dates = sorted({e['session_date'] for e in entries}); symbols = sorted({e['symbol'] for e in entries})
    manifest = dict(schema='compact_preview_snapshot_v1', state='complete', label=label,
        scope='completed_symbol_day_snapshot', cutoff_unix=cutoff,
        ...
        partitions=len(entries), full_partitions=len(entries), partial_partitions=0,
        rows=57600*len(entries), dates=dates, date_min=dates[0], date_max=dates[-1], symbols=symbols,
        inventory_sha256=P.file_identity(output/'inventory.jsonl')['sha256'],
        provenance=dict(layout_version=P.S.LAYOUT_VERSION, feature_contract=P.S.FEATURE_CONTRACT,
                        calculation=calc, calculation_identity=P.digest(calc)),
        run=str(Path(run).resolve()) if run else None, observed_states=dict(counts), exclusions=exclusions)
    P.atomic_json(output/'manifest.json', manifest)
    return manifest


def read(snapshot):
    snapshot = Path(snapshot); m = read_json(snapshot/'manifest.json')
    ...
    if m['provenance']['calculation_identity'] != P.digest(m['provenance']['calculation']):
    ...
    if len(keys) != len(entries) or len(entries) != m['partitions'] or sum(e['expected_rows'] for e in entries) != m['rows']:
