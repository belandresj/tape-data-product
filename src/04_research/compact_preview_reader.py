...
import report_release_inventory as REPORT


def columns_for(features):
    if not features or len(set(features)) != len(features) or set(features) - set(S.FEATURES):
        raise ValueError('select unique compact numerical features')
    return [*S.KEYS, 'post_discovery_eligible', *features, *(f+'_reason_mask' for f in features)]


def stats_new():
    return dict(bytes_transferred=0, http_requests=0, head_requests=0, get_requests=0,
        whole_feature_bytes=0, projected_chunk_bytes=0, marker_bytes=0,
        decode_seconds=0., validation_seconds=0., transfer_seconds=0.,
        scratch_peak_bytes=0, rows=0, mode='whole-feature-transfer/projected-decode')
...
def get_response(client, bucket, obj, stats):
    ...
    """
    stats['http_requests'] += 1; stats['get_requests'] += 1
    response = client.get_object(Bucket=bucket, Key=obj['object_key'])
    meta = response.get('Metadata', {})
    if (not response.get('ETag') or response.get('ContentLength') != obj['size_bytes']
...
def fetch_manifest(client, entry, accepted, stats):
    start = time.perf_counter(); obj = entry['objects']['manifest']
    if obj['size_bytes'] > 256*1024: raise ValueError('completion exceeds bound')
    response = get_response(client, entry['bucket'], obj, stats)
    try: raw = response['Body'].read(obj['size_bytes']+1)
    finally: response['Body'].close()
    stats['bytes_transferred'] += len(raw); stats['marker_bytes'] += len(raw)
    if len(raw) != obj['size_bytes'] or hashlib.sha256(raw).hexdigest() != obj['sha256']:
        raise ValueError('completion marker SHA/length mismatch')
    stats['transfer_seconds'] += time.perf_counter()-start
    start = time.perf_counter(); manifest = json.loads(raw)
...
def validate_batch(batch, features, entry, meta, offset, include_diagnostics=False):
    ...
    for name, expected in [('interval_end_ns', ends), ('session_date', entry['session_date']), ('symbol', entry['symbol'])]:
        if not np.all(arrays[name] == expected): raise ValueError('timestamp grid/key mismatch: '+name)
    post = arrays['post_discovery_eligible']
    endpoint = meta['discovery']['first_discovery_endpoint_ns']
    expected_post = ends >= endpoint if meta['discovery_verified'] and endpoint is not None else np.zeros(n, bool)
    if not np.array_equal(post, expected_post): raise ValueError('post-discovery mask mismatch')
    values = {}
    ...
    for f in features:
        ...
        if np.any(~np.isfinite(value) & ~batch[f].is_null().to_numpy(zero_copy_only=False)):
        ...
        if f.startswith('movement_participation_') and np.any((value[eligible] < 0) | (value[eligible] > 1)):
...
def local_batches(path, entry, meta, features, stats, batch_size=4096, include_diagnostics=False):
    if type(batch_size) is not int or not 1 <= batch_size <= 25000:
        raise ValueError('batch_size must be 1..25000')
    cols = columns_for(features); offset = 0
    with pq.ParquetFile(path) as pf:
        ...
        for rg in range(pf.metadata.num_row_groups):
            ...
            if group.num_rows > 25000: raise ValueError('unbounded Parquet row group')
        ...
        while True:
            ...
            try: batch = next(it)
                ...
                stats['decode_seconds'] += time.perf_counter()-start; break
            stats['decode_seconds'] += time.perf_counter()-start
            start = time.perf_counter()
            result = validate_batch(batch, features, entry, meta, offset, include_diagnostics)
            stats['validation_seconds'] += time.perf_counter()-start
            offset += batch.num_rows; stats['rows'] += batch.num_rows
            yield result
            del batch, result
    if offset != entry['expected_rows']: raise ValueError('incomplete timestamp grid')


def batches(client, entry, accepted, features, scratch, stats, batch_size=4096, manifest_sink=None, include_diagnostics=False):
    cols = columns_for(features)
    manifest = fetch_manifest(client, entry, accepted, stats)
    if manifest_sink is not None: manifest_sink.update(manifest)
    obj = entry['objects']['features']; scratch = Path(scratch); scratch.mkdir(parents=True, exist_ok=True)
    if obj['size_bytes'] > 128*1024**2: raise ValueError('feature scratch exceeds 128 MiB bound')
    # The file is created by this reader. Failed reads retain it for diagnosis.
    with tempfile.NamedTemporaryFile(prefix='preview-feature-', suffix='.parquet', dir=scratch, delete=False) as out:
        path = Path(out.name); start = time.perf_counter()
        response = get_response(client, entry['bucket'], obj, stats)
        sha = hashlib.sha256(); size = 0
        try:
            while block := response['Body'].read(1024**2):
                size += len(block); stats['bytes_transferred'] += len(block)
                if size > obj['size_bytes']: raise ValueError('feature body exceeds declared length')
        ...
        stats['transfer_seconds'] += time.perf_counter()-start
    yield from local_batches(path, entry, manifest['metadata'], features, stats, batch_size,
