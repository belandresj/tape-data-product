    ...
    *,
    ...
    try:
        with ThreadPoolExecutor(
            max_workers=min(download_workers, len(STREAMS))
        ) as pool:
            futures = {
                pool.submit(
    ...
    for session_date in dates:
        ...
        if not date_root.is_dir() or date_root.is_symlink():
            raise FileNotFoundError(
                f"local date directory missing or unsafe: {date_root}"
            )
        date_entries = sorted(date_root.iterdir())
        unexpected_date_entries = [
            path.name for path in date_entries if not path.is_dir()
        ]
        ...
        for symbol_dir in symbol_dirs:
            ...
            if symbol_dir.is_symlink():
                raise RuntimeError(
                    f"local symbol directory may not be a symlink: {symbol_dir}"
                )
            unexpected = sorted(
                path.name
            ...
            if unexpected:
                raise RuntimeError(
                    f"unexpected local T/Q files in {symbol_dir}: {unexpected}"
                )
            for stream in STREAMS:
                path = symbol_dir / f"{stream}.parquet"
                if not path.is_file() or path.is_symlink():
                    raise FileNotFoundError(
                        f"local T/Q artifact missing or unsafe: {path}"
                    )
...
def delete_verified_local_dates(session_root: Path, dates: Sequence[str]) -> None:
    ...
    for session_date in dates:
        ...
        date_root = session_root / session_date
        if (
            date_root.parent != session_root
            or not date_root.is_dir()
            or date_root.is_symlink()
        ):
...
def main() -> int:
    ...
    if args.command == "audit-local-tq":
        ...
        if args.delete_local_after_verify:
            ...
            print("deleted_verified_local_dates=" + ",".join(args.dates))
        return 0
