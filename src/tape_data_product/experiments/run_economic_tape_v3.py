...
def file_identity(path):
    ...
    return dict(
        ...
        rows=V3.pq.ParquetFile(path).metadata.num_rows,
        sha256=V3.sha256(path),
    )


def load_halts(directory, day, symbol):
    """Verify the frozen registry using the existing authoritative verifier."""
    from tape_data_product.features import historical_halt_registry_v1 as module
    ...
    for name in ("registry.parquet", "accepted_intervals.parquet"):
        if V3.pq.ParquetFile(directory / name).metadata.num_rows > 25_000:
            raise ValueError(
                "registry exceeds metadata bound; use a bounded registry verifier"
            )
    manifest = module.verify_frozen_registry(directory)
    intervals = []
    for batch in V3.pq.ParquetFile(
        directory / "accepted_intervals.parquet"
    ).iter_batches(batch_size=1024, use_threads=False):
        for row in batch.to_pylist():
            ...
            intervals.append(
                (
                    a,
                    int(b) if b is not None else V3.session_bounds(day)[1],
                    row["halt_interval_id"],
                )
            )
    return intervals, dict(
        ...
        accepted_interval_ids=[x[2] for x in intervals],
...
def require_space(path, required):
    ...
    if free < required:
        raise OSError(
            f"insufficient scratch disk: {free} bytes free; {required} required"
        )
...
def worker(args):
    ...
    manifest = dict(
        ...
        plan_sha256=V3.sha256(args.plan),
        sources=[],
        full_run_approved_flag=args.full_run_approved,
    )
    atomic_json(args.output / "run_manifest.json", manifest)
    try:
        for source in sources:
            day, symbol = storage._validate_date_symbol(
                source["session_date"], source["symbol"]
            )
            ...
            if (
                coverage.get("trades") != "[04:00,20:00) ET"
                or coverage.get("quotes") != "[03:55,20:00) ET"
            ):
            ...
            if day not in source.get("halt_coverage_dates", []) or not source.get(
                "halt_coverage_provenance"
            ):
                raise ValueError(
                    "source lacks explicit halt registry date coverage/provenance"
                )
            ...
            if remote:
                ...
                for stream in ("trades", "quotes"):
                    ...
                    remote_heads[stream] = dict(
                        identity=asdict(identity), metadata=head.get("Metadata", {})
                    )
                require_space(args.scratch_root, required + 2 * 1024**3)
                context = storage.staged_symbol_day(
                    client,
                    settings.bucket,
                    day,
                    symbol,
            ...
            with context as staged:
                ...
                identity.update(
                    raw_sources={
                        "trades": file_identity(trades),
                        "quotes": file_identity(quotes),
                    },
                    coverage=coverage,
                    coverage_provenance=source["coverage_provenance"],
                    halt_registry=halt_identity,
                    remote_objects=remote_heads,
                    ...
                    sample_seconds=args.sample_seconds,
                    source_acceptance=(
                        "prefix-only" if args.sample_seconds else "complete-file"
                    ),
                    universe_provenance=plan["universe_provenance"],
                ...
                result = V3.write_features(
                    ...
                    identity=identity,
                    halts=halts,
                    seconds=args.sample_seconds or 57_600,
        ...
        if not args.features_only:
            ...
            reference_rows = sum(
                ...
                if s["session_date"] == NN.REFERENCE_DATE
            )
            ...
            with tempfile.TemporaryDirectory(
                prefix="v3-search-", dir=args.scratch_root
            ) as work:
                result = NN.run(
                    paths[NN.REFERENCE_DATE],
                    paths[NN.QUERY_DATE],
                    Path(work) / "neighbors",
        ...
        manifest["status"] = (
            "bounded_sample_complete" if args.sample_seconds else "complete"
        )
...
def monitored(command, measurement_path):
    ...
    peak, worker_peak, supervisor_peak, killed, started = (
        0,
        0,
        0,
        False,
    ...
    try:
        ...
        atomic_json(
            ...
            dict(
                ...
                peak_worker_tree_rss_bytes=worker_peak,
                peak_supervisor_rss_bytes=supervisor_peak,
                stop_threshold_bytes=STOP_BYTES,
                killed_for_memory=killed,
                elapsed_seconds=time.monotonic() - started,
...
def main():
    ...
    if args.worker:
        ...
        with tempfile.TemporaryDirectory(
            prefix="v3-run-", dir=args.scratch_root
        ) as private:
            command = [
                sys.executable,
            ...
            raise SystemExit(
                monitored(
                    command,
                    args.output.parent
                    / f"{args.output.name}_resource_measurement.json",
                )
            )
