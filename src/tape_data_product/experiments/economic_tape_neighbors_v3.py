...
from tape_data_product.features import economic_tape_state_v3 as V3
...
def session_segment(day, end):
    ...
    return (
        "premarket"
        if elapsed <= 19_800 * V3.NS
        else "rth" if elapsed <= 43_200 * V3.NS else "after_hours"
    )
...
def feature_rows(paths, expected_day, expected_contract=None):
    ...
    for path in sorted(map(Path, paths)):
        ...
        if (
            meta.get("version") != V3.VERSION
            or meta.get("spread_version") != V3.SPREAD_VERSION
        ):
        ...
        cols = [
            ...
            "symbol",
            "interval_end_ns",
            "pilot_base_comparison_eligible",
            *V3.FEATURES,
        ]
        previous = None
        for batch in pf.iter_batches(batch_size=4096, columns=cols, use_threads=False):
            for row in batch.to_pylist():
                ...
                if not all(
                    v is not None and math.isfinite(v) and v >= 0 for v in values
                ):
...
def load_population(db, table, paths, day, contract):
    ...
    db.execute(
        f"CREATE TABLE {table}(symbol TEXT, endpoint INTEGER, priority TEXT, {columns}, PRIMARY KEY(symbol, endpoint))"
    )
    ...
    for symbol, end, values in feature_rows(paths, day, contract):
        ...
        priority = hashlib.sha256(
            f"v3-pilot-anchor-1|{day}|{symbol}|{end}".encode()
        ).hexdigest()
...
def fit_scaler(db, count, contract):
    ...
    for j in range(18):
        ...
        for probability in (0.25, 0.5, 0.75):
            ...
            values = [
                db.execute(
                    f"SELECT x{j} FROM reference ORDER BY x{j} LIMIT 1 OFFSET ?",
                    (offset,),
                ).fetchone()[0]
            ...
            weight = pos - lower
            quantiles.append(
                math.log1p(values[0]) * (1 - weight) + math.log1p(values[1]) * weight
            )
            ...
            (
                ...
                if "age_p90" in x
                    ...
                    if "bps" in x
                        ...
                        "1 USD/s"
                        if "dollar_rate" in x
                        else "1 USD" if "notional" in x else "1 event/s"
                    )
                )
            )
            for x in V3.FEATURES
        ],
        unit_values=[1.0] * 18,
        median=medians,
        iqr=iqrs,
        eligible_reference_rows=count,
...
def nearest(db, queries, scaler, k=5):
    ...
    while rows := cursor.fetchmany(REFERENCE_BLOCK):
        ...
        for i in range(len(queries)):
            ...
            best[i].extend(
                (float(distance[i, n]), rows[n][0], rows[n][1], rows[n][3:])
                for n in candidates
            )
...
def run(reference_paths, query_paths, output_dir, *, max_queries=128, k=5):
    ...
    contract = json.loads(
        pq.ParquetFile(reference_paths[0]).schema_arrow.metadata[b"v3_identity"]
    )["contract_hash"]
    output_dir.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(
        prefix=".v3-neighbors-", dir=output_dir.parent
    ) as scratch:
        scratch = Path(scratch)
        db = connect(scratch / "populations.sqlite")
        try:
            references = load_population(
                db, "reference", reference_paths, REFERENCE_DATE, contract
            )
            candidates = load_population(
                db, "queries", query_paths, QUERY_DATE, contract
            )
            if candidates == 0:
                raise ValueError("no eligible fixed-grid September 2 query anchors")
            scaler = fit_scaler(db, references, contract)
            source_identity = {
                str(Path(p).resolve()): V3.sha256(p)
                for p in [*reference_paths, *query_paths]
            }
            scaler["reference_artifact_sha256"] = {
                str(Path(p).resolve()): source_identity[str(Path(p).resolve())]
                for p in reference_paths
            }
            (scratch / "scaler.json").write_text(json.dumps(scaler, indent=2) + "\n")
            cursor = db.execute(
                "SELECT * FROM queries ORDER BY priority, symbol, endpoint LIMIT ?",
                (max_queries,),
            )
            ...
            segment_counts = {}
            db.execute(
                "CREATE TABLE matched_blocks(symbol TEXT, block INTEGER, PRIMARY KEY(symbol,block))"
            )
                ...
                while queries := cursor.fetchmany(QUERY_BLOCK):
                    for query, matches in nearest(db, queries, scaler, k):
                        ...
                        for rank, (_, symbol, end, values) in enumerate(matches, 1):
                            overall, families, contributions = decompose(
                                query[3:], values, scaler
                            )
                            overlap = any(
                                s == symbol and abs(t - end) <= 300 * V3.NS
                                for s, t in prior_matches
                            )
                            ...
                            block = (end - V3.session_bounds(REFERENCE_DATE)[0]) // (
                                300 * V3.NS
                            )
                            db.execute(
                                "INSERT OR IGNORE INTO matched_blocks VALUES (?,?)",
                                (symbol, block),
                            )
                            row = dict(
                                query_date=QUERY_DATE,
                                query_symbol=query[0],
                                query_interval_end_ns=query[1],
                                reference_date=REFERENCE_DATE,
                                reference_symbol=symbol,
                                reference_interval_end_ns=end,
                                rank=rank,
                                D_base=overall,
                                family_distances=families,
                                weighted_squared_contributions=dict(
                                    zip(V3.FEATURES, contributions)
                                ),
                                query_raw=dict(zip(V3.FEATURES, query[3:])),
                                reference_raw=dict(zip(V3.FEATURES, values)),
                                overlaps_higher_ranked_reference=overlap,
                                reference_block_5m=block,
                                reference_session_segment=segment,
                                query_session_segment=session_segment(
                                    QUERY_DATE, query[1]
                                ),
                            )
