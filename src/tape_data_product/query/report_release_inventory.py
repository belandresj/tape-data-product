...
from tape_data_product.features import compact_preview_inventory as PREVIEW
...
def validation_level(metadata, evidence, expected_rows=57600):
    ...
    if policy is None:
        if (
            evidence.get("mode") is not None
            or evidence.get("independent_reconstruction") is not None
            or evidence.get("rows_integrity_checked") is not None
        ):
            raise ValueError("contradictory legacy validation evidence")
        if set(fields) != set(S.FEATURES):
            raise ValueError(
                "legacy completion lacks exhaustive reconstruction evidence"
            )
        return dict(
            policy_version="legacy_exhaustive_v1",
    ...
    if (
        ...
        or evidence.get("rows_integrity_checked") != expected_rows
    ):
    ...
    if mode == "integrity":
        ...
        set(fields) != set(S.FEATURES)
        or evidence.get("independent_reconstruction") != "passed"
    ):
...
def manifest_compatibility(manifest, entry, accepted_calculations):
    ...
    if (
        meta.get("layout_version") != S.LAYOUT_VERSION
    ...
    if P.digest(meta) != entry.get("partition_identity") or manifest.get(
        "partition_identity"
    ...
    for name, schema in (("features", S.FEATURE_SCHEMA), ("support", S.SUPPORT_SCHEMA)):
        ...
        if obj.get("file") != name + ".parquet" or obj.get(
            "schema_hash"
        ) != S.schema_hash(schema):
            raise ValueError("unsupported object schema/filename")
        if any(
            obj.get(key) != frozen.get(key) for key in ("sha256", "size_bytes", "rows")
        ):
...
def _selection_compatibility(selected, metadata):
    ...
    if selected.get("partition_identity") is not None and selected[
        "partition_identity"
    ] != P.digest(metadata):
    ...
    if "discovery" in selected:
        ...
        if metadata.get("discovery") != expected or metadata.get(
            "discovery_verified"
        ) != bool(discovery.get("verified")):
    ...
    for item in _records(path):
        ...
        if disposition == "selected":
            ...
            if "discovery" in selected[key]:
                selected[key]["discovery"] = {
                    k: v for k, v in item["discovery"].items() if k != "provenance"
                }
        elif (
...
def _publication(record):
    ...
    if not isinstance(manifest, dict):
        raise ValueError(
            "identity-verified completion manifest required for compatibility decision"
        )
...
def freeze(
    ...
    *,
    ...
    """
    accepted, accepted_ids = _accepted(accepted_calculations)
    selection_id, intended, explicit_exclusions = _selection(
    ...
    ledger = [
        ...
        for row in explicit_exclusions
    ]
        ...
        if not found:
            ...
            ledger.append(
                dict(
                    ...
                    reason=(
                        "malformed completed publication record(s): "
                        + "; ".join(p["reason"] for p in problems)
                        if problems
                        else "no completed publication captured"
                    ),
                )
            )
            continue
        if len(found) != 1 or malformed_by_key.get(key):
            ledger.append(
                dict(
                    ...
                    symbol=key[1],
                    status="incompatible",
                    reason="ambiguous competing completed partitions",
    ...
    for key, found in sorted(candidates.items()):
        ledger.append(
            dict(
                ...
                symbol=key[1],
                status="excluded",
                reason="completed publication is outside intended selection",
                candidates=len(found),
            )
        )
    ...
    for path, rows in (
        (accepted_path, accepted_entries),
        (ledger_path, ledger),
    ...
    validation_counts = Counter(
        entry["validation"]["mode"] for entry in accepted_entries
    )
    manifest = dict(
        schema=SCHEMA,
        state="frozen",
        release_status="complete" if unresolved == 0 else "incomplete",
        label=label,
        selection=dict(
            path=str(Path(selection).resolve()),
            sha256=selection_id,
            selected=len(intended),
            explicitly_excluded=len(explicit_exclusions),
        ),
        publication_capture=dict(
            path=str(Path(publications).resolve()),
            sha256=_digest_file(publications),
            malformed_records=len(malformed),
            capture_exclusions_sha256=_digest_file(exclusions_path),
        ),
        compatibility_policy=dict(
            layout_version=S.LAYOUT_VERSION,
            feature_contract=S.FEATURE_CONTRACT,
            builder_version=P.BUILDER_VERSION,
            feature_schema_hash=S.schema_hash(S.FEATURE_SCHEMA),
        ...
        accepted_members=len(accepted_entries),
        rows=sum(e["expected_rows"] for e in accepted_entries),
        symbol_days=len(accepted_entries),
        dates=dates,
        date_min=dates[0] if dates else None,
        date_max=dates[-1] if dates else None,
        symbols=symbols,
        calculation_identities=calculations,
        validation_counts=dict(validation_counts),
        accepted_sha256=_digest_file(accepted_path),
        reconciliation_sha256=_digest_file(ledger_path),
    )
    manifest["release_identity"] = P.digest(
        {key: manifest[key] for key in manifest if key != "release_identity"}
    )
...
def read(directory, *, accepted_calculations=None, require_complete=True):
    ...
    if _digest_file(directory / "accepted.jsonl") != manifest.get(
        "accepted_sha256"
    ) or _digest_file(directory / "reconciliation.jsonl") != manifest.get(
        "reconciliation_sha256"
    ):
        raise ValueError("report release member identity mismatch")
    capture = manifest.get("publication_capture", {})
    if _digest_file(directory / "capture_exclusions.jsonl") != capture.get(
        "capture_exclusions_sha256"
    ):
    ...
    if (
        ...
        or len(entries) != manifest["accepted_members"]
        or sum(entry["expected_rows"] for entry in entries) != manifest["rows"]
    ):
        raise ValueError("report release coverage mismatch")
    for entry in entries:
        if (
            entry.get("calculation_identity")
            not in manifest["compatibility_policy"]["accepted_calculation_identities"]
        ):
            raise ValueError("accepted member calculation identity mismatch")
    return manifest, entries


def iter_accepted_members(
    directory, *, accepted_calculations=None, require_complete=True
):
...
def projected_feature_batches(
    ...
    *,
    ...
    _, entries = read(
        directory, accepted_calculations=accepted, require_complete=require_complete
    )
    stats = stats if stats is not None else READER.stats_new()
    for entry in entries:
        for batch in READER.batches(
            client, entry, accepted, features, scratch, stats, batch_size=batch_size
        ):
            yield entry, batch
...
def capture_run(run, output, *, accepted_calculations):
    ...
    with output.open("x") as handle:
        for path in sorted((run / "receipts").glob("*.json")):
            raw = path.read_bytes()
            if len(raw) > MAX_RECORD:
                raise ValueError("receipt exceeds control bound")
            ...
            if len(line) > MAX_RECORD:
                raise ValueError("publication capture record exceeds control bound")
            handle.write(line)
            count += 1
...
def main():
    ...
    if args.action == "capture":
        if args.run is None:
            parser.error("capture requires --run")
        ...
        if not all((args.selection, args.selection_sha256, args.publications)):
            parser.error(
                "freeze requires --selection, --selection-sha256, --publications"
            )
        result = freeze(
            args.selection,
            args.publications,
            ...
            accepted_calculations=accepted,
        )
