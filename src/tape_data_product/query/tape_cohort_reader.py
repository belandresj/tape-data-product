...
def projected_columns(config, include_midpoint=False):
    ...
    features = [c["feature"] for c in config["conditions"]]
    return [
        *schema.KEYS,
        *features,
...
def _requires(feature):
    ...
    return (
        ("quote", "trade")
        if feature.startswith("movement_mean_to_spread_")
        else ("quote",)
    )
...
def _validate_batch(batch, entry, metadata, features, offset):
    n = batch.num_rows
    arrays = {
        name: batch[name].to_numpy(zero_copy_only=False) for name in batch.schema.names
    }
    expected_end = (
        CORE.session_start(entry["session_date"])
        + (np.arange(offset, offset + n, dtype=np.int64) + 1) * CORE.NS
    )
    if not np.array_equal(arrays["interval_end_ns"], expected_end):
        raise ValueError("timestamp grid mismatch")
    if not np.all(arrays["session_date"] == entry["session_date"]):
        raise ValueError("session_date key mismatch")
    if not np.all(arrays["symbol"] == entry["symbol"]):
        raise ValueError("symbol key mismatch")
    discovery = metadata.get("discovery", {})
    endpoint = discovery.get("first_discovery_endpoint_ns")
    expected_post = (
        expected_end >= endpoint
        if metadata.get("discovery_verified") and endpoint is not None
        else np.zeros(n, bool)
    )
    if not np.array_equal(arrays["post_discovery_eligible"], expected_post):
        raise ValueError("post-discovery mask mismatch")
    for name in (*CONTEXT,):
        if batch[name].null_count:
            raise ValueError("null common context")
    for feature in features:
        masks = arrays[feature + "_reason_mask"]
        if (
            batch[feature + "_reason_mask"].null_count
            or masks.dtype.kind not in "iu"
            or np.any(masks < 0)
            or np.any(masks & ~KNOWN_MASK)
        ):
        ...
        if np.any(~nulls & ~np.isfinite(values)):
            raise ValueError("stored nonfinite feature: " + feature)
        zero = masks == 0
        if np.any(zero & (nulls | (values < 0))):
            raise ValueError("zero-mask value contradiction: " + feature)
        if feature.startswith("movement_participation_") and np.any(
            zero & (values > 1)
        ):
        ...
        for dependency in _requires(feature):
            ...
            if np.any(zero & ~accepted):
                raise ValueError("zero mask contradicts required source")
        if np.any(zero & arrays["halt_interval_active"]):
            raise ValueError("zero mask contradicts active halt")
    for horizon in ("60s", "300s"):
        m, spread, ratio = (
            f"movement_mean_5s_bps_{horizon}",
            f"quoted_spread_mean_bps_{horizon}",
            f"movement_mean_to_spread_{horizon}",
        )
        if {m, spread, ratio} <= set(features):
            valid = (
                (arrays[m + "_reason_mask"] == 0)
                & (arrays[spread + "_reason_mask"] == 0)
                & (arrays[ratio + "_reason_mask"] == 0)
            )
            if np.any(
                valid
                & ~np.isclose(
                    arrays[ratio], arrays[m] / arrays[spread], rtol=1e-10, atol=1e-12
                )
            ):
...
def inspect_physical(path, entry, columns):
    ...
    if not pf.schema_arrow.equals(schema.FEATURE_SCHEMA, check_metadata=False):
        raise ValueError("physical feature schema mismatch")
    for rg in range(pf.metadata.num_row_groups):
        group = pf.metadata.row_group(rg)
        if group.num_rows > MAX_BATCH:
        ...
        for i in range(group.num_columns):
            ...
            if chunk.path_in_schema in columns:
                if chunk.total_uncompressed_size > MAX_CHUNK:
                    raise ValueError("unbounded projected column chunk")
                projected += chunk.total_uncompressed_size
        if projected > MAX_PROJECTED_ROW_GROUP:
            raise ValueError("unbounded projected row group")
    return pf
...
def iter_query_batches(
    ...
    *,
    ...
    if max_rows is not None and (
        type(max_rows) is not int or not 1 <= max_rows <= member["expected_rows"]
    ):
        raise ValueError("invalid checkpoint prefix")
    config = normalize_config(config)
    features = [c["feature"] for c in config["conditions"]]
    columns = projected_columns(config, include_midpoint)
    stats = stats or ValidationStats()
    stats.validation_scope = "prefix" if max_rows is not None else "full"
    stats.prefix_count = max_rows
    pf = inspect_physical(Path(path), member, columns)
    offset = 0
    for batch in pf.iter_batches(
        ...
        for name in columns:
            if batch.schema.field(name) != schema.FEATURE_SCHEMA.field(name):
    ...
    if offset != expected:
        raise ValueError("incomplete projected grid")
