...
from tape_data_product.query.tape_cohort_outputs import verify_date
...
def worked_transition_oracle():
    ...
    machine = CohortMachine(config, "0" * 64)
        ...
        machine.consume_row(
            {
                ...
                "trade_rate_60s": value,
                "trade_rate_60s_reason_mask": 0,
            },
            sinks,
        )
    machine.finish("selection_boundary", sinks)
    if len(sinks["windows"]) != 1:
        raise AssertionError("worked oracle window count")
    w = sinks["windows"][0]
    expected = {
        "candidate_start_endpoint_ns": 4_000_000_000,
        "entry_confirmed_at_ns": 6_000_000_000,
        "exit_trigger_endpoint_ns": 10_000_000_000,
        "exit_effective_at_ns": 12_000_000_000,
        "active_seconds": 6,
        "strict_pass_count": 1,
        "continuation_pass_count": 3,
        "pending_exit_count": 3,
    }
    if any(w[k] != v for k, v in expected.items()):
        raise AssertionError({k: (w[k], v) for k, v in expected.items() if w[k] != v})
    return expected
...
def verify_run(run):
            ...
            verified.append(
                {
                    ...
                    **verify_date(
                        ...
                        date_manifest,
                        len(
                            json.loads((root / "query.json").read_text())["conditions"]
                        ),
                    ),
                }
            )
    return {
        "state": "verified",
        "dates": verified,
        "worked_oracle": worked_transition_oracle(),
    }
...
def main():
    ...
    print(
        json.dumps(
            worked_transition_oracle() if a.oracle else verify_run(a.run), indent=2
        )
    )
