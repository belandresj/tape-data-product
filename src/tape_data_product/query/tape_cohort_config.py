...
UNITS = {name: meta["unit"] for name, meta in schema.field_metadata().items()}
FEATURE_INDEX = {name: i for i, name in enumerate(schema.FEATURES)}
REASON_BITS = dict(schema.REASONS)
...
def _bounds(raw, inherited, label):
    ...
    if out["lower"] is not None and out["upper"] is not None:
        if out["lower"] > out["upper"] or (
            out["lower"] == out["upper"]
            and not (out["lower_inclusive"] and out["upper_inclusive"])
        ):
...
def _inside(entry, continuation):
    ...
    if cl is not None and (
        ...
        or (
            el == cl
            and entry["lower_inclusive"]
            and not continuation["lower_inclusive"]
        )
    ):
        return False
    if cu is not None and (
        ...
        or (
            eu == cu
            and entry["upper_inclusive"]
            and not continuation["upper_inclusive"]
        )
    ):
        return False
    return True
...
INITIAL_CONFIG = normalize_config(
    {
        ...
        "conditions": [
            ...
            {
                "feature": "quoted_spread_mean_bps_60s",
                "unit": "bps",
                "entry": {
                    "lower": 0.0,
            ...
            {
                "feature": "movement_mean_to_spread_60s",
                "unit": "dimensionless",
                "entry": {
                    "lower": 1.5,
                    "lower_inclusive": True,
