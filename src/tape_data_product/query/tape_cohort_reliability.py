...
def transient(exc):
    ...
    if isinstance(exc, ClientError):
        ...
        return response.get("ResponseMetadata", {}).get("HTTPStatusCode") in (
            429,
            500,
            502,
            503,
            504,
        ) or response.get("Error", {}).get("Code") in (
            "SlowDown",
            "RequestTimeout",
            "InternalError",
            "ServiceUnavailable",
...
def fatal(exc):
    ...
    if isinstance(
        exc, (MemoryError, OSError, NoCredentialsError, PartialCredentialsError)
    ) and not transient(exc):
        return True
    if isinstance(exc, ClientError):
        return exc.response.get("ResponseMetadata", {}).get("HTTPStatusCode") in (
            401,
            403,
        )
...
def journal(path, event):
    with pathlib.Path(path).open("a") as stream:
        stream.write(
            json.dumps({"time_ns": time.time_ns(), **event}, sort_keys=True) + "\n"
        )
        stream.flush()
...
def retry_download(operation, partial, log, *, sleep=time.sleep):
    ...
    for attempt in range(1, 4):
        try:
            ...
            journal(
                log,
                {"attempt": attempt, "error_type": type(exc).__name__, "retry": retry},
            )
            if not retry:
                raise
...
def supervise(plan_path, *, resume=False):
    ...
    from tape_data_product.query.tape_cohort_pipeline import _load_plan
    from tape_data_product.query.tape_cohort_outputs import atomic_json

    plan = _load_plan(plan_path)
    root = pathlib.Path(plan["result_root"])
    settings = plan["settings"]
    root.mkdir(parents=True, exist_ok=True)
    with (root / (plan["query_run_hash"] + ".supervisor.lock")).open("a") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        stamp = str(time.time_ns())
        logpath = root / ("run-" + stamp + ".log")
        resource = root / ("resources-" + stamp + ".json")
        cmd = [
            sys.executable,
            "-B",
            str(pathlib.Path(__file__).with_name("run_tape_cohort_query.py")),
            "run",
            "--worker",
            "--plan",
            str(pathlib.Path(plan_path).resolve()),
        ]
        if resume:
            cmd.append("--resume")
        ...
        with logpath.open("x") as log:
            ...
            child = subprocess.Popen(
                cmd, stdout=log, stderr=subprocess.STDOUT, env=environment
            )
            try:
                while child.poll() is None:
                    ...
                    try:
                        ...
                        rss = psutil.Process().memory_info().rss + sum(
                            x.memory_info().rss
                    ...
                    if peak >= settings["rss_stop_bytes"]:
                        reason = "rss_limit"
                    elif available < settings["minimum_available_bytes"]:
                ...
                if code or reason:
                    ...
                    if completion.exists():
                        ...
                        if value.get("state") != "complete":
                            value.update(
                                state="incomplete",
                                supervisor_stop_reason=reason or "worker_error",
                            )
                ...
                atomic_json(
                    resource,
                    {
                        ...
                        "maximum_monitor_gap_seconds": gap,
                        "elapsed_seconds": time.monotonic() - start,
                        "log": str(logpath),
                    },
                )
        return {
            "exit_code": code or (1 if reason else 0),
