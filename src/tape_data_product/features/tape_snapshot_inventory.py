...
def validate_metadata(day, stream, metadata):
    ...
    if (
        metadata.get("session-window-et") != expected
        or metadata.get("pagination-complete", "true") != "true"
    ):
    ...
    start = (
        int(
            datetime.combine(
                date.fromisoformat(day), a, ZoneInfo("America/New_York")
            ).timestamp()
        )
        * 10**9
    )
        ...
        int(
            datetime.combine(
                date.fromisoformat(day), time(20), ZoneInfo("America/New_York")
            ).timestamp()
        )
        * 10**9
    )
    if (
        "requested-start-ns" in metadata
        and int(metadata["requested-start-ns"]) != start
    ) or ("requested-end-ns" in metadata and int(metadata["requested-end-ns"]) != end):
...
def freeze(output):
    ...
    if listing.exists():
        ...
        for page in client.get_paginator("list_objects_v2").paginate(
            Bucket=settings.bucket, Prefix="tq/"
        ):
            for obj in page.get("Contents", []):
                ...
                day, symbol = STORAGE._validate_date_symbol(
                    parts[1].split("=", 1)[1], parts[2].split("=", 1)[1]
                )
                pairs[(day, symbol)][parts[-1].split(".")[0]] = dict(
                    key=obj["Key"], bytes=obj["Size"], etag=obj["ETag"]
                )
        record = dict(
            listed_at_utc=datetime.now(timezone.utc).isoformat(),
            bucket=settings.bucket,
            complete_pairs=[
                dict(session_date=d, symbol=s, objects=v)
                for (d, s), v in sorted(pairs.items())
                if len(v) == 2
            ],
            incomplete_pairs=[
                dict(session_date=d, symbol=s, objects=v)
    ...
    def verify(source):
        ...
        try:
            for stream, item in source["objects"].items():
                ...
                if (
                    head["ETag"] != item["etag"]
                    or head["ContentLength"] != item["bytes"]
                ):
                ...
                validate_metadata(
                    source["session_date"], stream, head.get("Metadata", {})
                )
                result[stream] = dict(
                    **asdict(identity),
                    ...
                    etag=head["ETag"],
                    coverage_evidence=(
                        "requested_bounds_and_complete_pagination"
                        if "pagination-complete" in head["Metadata"]
                        else "legacy_canonical_session_window_declaration"
                    ),
                )
    ...
    reusable = {
        (s["session_date"], s["symbol"]): s
        for s in prior["sources"]
        if s["status"] == "accepted_canonical_metadata"
    }

    def verify_or_reuse(source):
        return reusable.get((source["session_date"], source["symbol"])) or verify(
            source
        )
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        for offset in range(0, len(record["complete_pairs"]), 8):
            sources.extend(
                pool.map(verify_or_reuse, record["complete_pairs"][offset : offset + 8])
            )
            if offset % 200 == 0:
                print(json.dumps({"verified_source_pairs": len(sources)}), flush=True)
    payload = dict(
        version="tape_source_snapshot_v2",
        bucket=record["bucket"],
        listed_at_utc=record["listed_at_utc"],
        listing_sha256=STORAGE.sha256_file(listing),
        sources=sources,
        incomplete_pairs=record["incomplete_pairs"],
        selection="initial complete-pair listing; accepted canonical acquisition metadata; no feature selection",
        ...
        json.dumps(
            {
                ...
                "accepted": sum(
                    x["status"] == "accepted_canonical_metadata"
