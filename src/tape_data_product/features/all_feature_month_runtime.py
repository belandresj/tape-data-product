...
def discover_incumbent(research, now=None):
    ...
    for path in Path(research).glob("tape_feature_queue*/status.json"):
        try:
            ...
            if abs(p.create_time() - state["process_created"]) > 0.01:
                continue
            cmd = p.cmdline()
            if str(path.parent.resolve()) not in cmd or not any(
                "run_tape_feature_queue" in x for x in cmd
            ):
                continue
            candidates.append(
                (
    ...
    if len(known) > 1:
        raise ResourceWait(
            "multiple live feature supervisors require explicit tree accounting"
        )
...
def tree(identity):
    ...
    try:
        ...
        if abs(p.create_time() - identity["created"]) > 0.01:
            return {}
        values = {}
        for x in [p, *p.children(recursive=True)]:
            try:
                values[x.pid] = x.memory_info().rss
            except psutil.NoSuchProcess:
                pass
        return values
    except psutil.NoSuchProcess:
...
def pressure(*, new_rss, incumbent_rss, available, free, needed=0):
    if new_rss >= 480 * MiB:
        return "new worker approaching 512 MiB stop"
    if available < GiB:
        return "system available memory below 1 GiB"
    if new_rss + incumbent_rss >= 2 * GiB:
        return "combined RSS reached 2 GiB"
    if free < 3 * GiB + needed:
        return "disk reserve/headroom unavailable"
    return None


def admit(research, scratch, projected_peak, needed=0, seconds=10, interval=0.1):
    ...
    peak_incumbent = 0
    identity = None
    rediscovered = -float("inf")
    current = None
    while True:
        now = time.monotonic()
        if now - rediscovered >= 1:
            current = discover_incumbent(research)
            rediscovered = now
        if identity is not None and current != identity:
            raise ResourceWait(
                "incumbent changed during admission; rediscover before another attempt"
            )
        identity = current
        incumbent_values = tree(current)
        if current and not incumbent_values:
            raise ResourceWait("incumbent exited or PID identity changed")
        ...
        if (
            available < 1.25 * GiB
            or incumbent + supervisor + 1.2 * projected_peak >= 2 * GiB
        ):
        ...
        if time.monotonic() - start >= seconds:
            return dict(
                ...
                peak_incumbent_rss_bytes=peak_incumbent,
            )
        time.sleep(interval)
...
def supervise(
    ...
    *,
    ...
    if marker.exists():
        raise ResourceWait("prior pressure marker requires reviewed resume")
    admission_seconds = (
        10 if admission_history is None else admission_history.seconds(research)
    )
    expected_incumbent = (
        admission_history.incumbent if admission_history is not None else None
    )
    if admission_history is not None:
        admission_history.clear()
    admission = admit(
        research, scratch, projected_peak, needed, seconds=admission_seconds
    )
    ...
    measurement = dict(
        admission=admission,
        peak_new_rss_bytes=0,
        peak_combined_rss_bytes=0,
        minimum_available_bytes=admission["minimum_available_bytes"],
        scratch_high_water_bytes=0,
        maximum_observation_gap_seconds=0,
        requested_observation_period_seconds=0.1,
    )
    ...
    with Path(report).with_suffix(".log").open("a") as log:
        ...
        owned = {}
        previous_observation = time.monotonic()
        current = admission["incumbent"]
        rediscovered = sized = reported = -float("inf")
        while child.poll() is None:
            observed = time.monotonic()
            measurement["maximum_observation_gap_seconds"] = max(
                measurement["maximum_observation_gap_seconds"],
            ...
            for pid in new:
                try:
                    ...
                    pass
            worker_rss = sum(new.values())
            supervisor_rss = psutil.Process().memory_info().rss
            new_rss = worker_rss + supervisor_rss
            measurement["peak_worker_rss_bytes"] = max(
                measurement.get("peak_worker_rss_bytes", 0), worker_rss
            )
            measurement["peak_supervisor_rss_bytes"] = max(
                measurement.get("peak_supervisor_rss_bytes", 0), supervisor_rss
            )
            try:
                if observed - rediscovered >= 1:
                    current = discover_incumbent(research)
                    rediscovered = observed
                incumbent_values = tree(current)
                if current and not incumbent_values:
                    raise ResourceWait("incumbent exited or PID identity changed")
                incumbent = sum(incumbent_values.values())
                if current != admission["incumbent"]:
                    reason = reason or "incumbent changed; yield before readmission"
            except ResourceWait as exc:
                incumbent = 0
                reason = reason or str(exc)
            available = psutil.virtual_memory().available
            free = shutil.disk_usage(scratch).free
            measurement["peak_new_rss_bytes"] = max(
                measurement["peak_new_rss_bytes"], new_rss
            )
            measurement["peak_combined_rss_bytes"] = max(
                measurement["peak_combined_rss_bytes"], new_rss + incumbent
            )
            measurement["minimum_available_bytes"] = min(
                measurement["minimum_available_bytes"], available
            )
            if observed - sized >= 1:
                measurement["scratch_high_water_bytes"] = max(
                    measurement["scratch_high_water_bytes"], scratch_bytes(scratch)
                )
                sized = observed
            ...
            reason = reason or pressure(
                new_rss=new_rss,
                incumbent_rss=incumbent,
                available=available,
                free=free,
                needed=needed,
            )
            if time.monotonic() - started > timeout:
                reason = reason or "worker timeout"
                ...
                try:
                    ...
                    for pid, created in owned.items():
                        try:
                            ...
                            pass
            time.sleep(max(0, 0.1 - (time.monotonic() - observed)))
        measurement["scratch_high_water_bytes"] = max(
            measurement["scratch_high_water_bytes"], scratch_bytes(scratch)
        )
        measurement.update(
            ...
            stop_reason=reason,
            os_memory_limit="macOS RLIMIT_AS not usable for shared mappings; independently monitored RSS",
            expensive_discovery_period_seconds=1,
            scratch_scan_period_seconds=1,
            target_met=measurement["peak_new_rss_bytes"] <= 384 * MiB,
        )
        ...
        if (
            ...
            and measurement["target_met"]
            and measurement["maximum_observation_gap_seconds"] <= 1
        ):
...
def check_yield():
    ...
    if path and Path(path).exists():
        raise ResourceWait(Path(path).read_text())
