...
from tape_data_product.features import tape_snapshot_inventory as I

S = I.STORAGE
SEMANTIC_PATH = F.ROOT / "resources/tape_snapshot_feature_semantics_v1.json"
...
def semantic_identity():
    ...
    return dict(
        ...
        contract_hash=I.digest(contract),
...
def partition_identity(entry, halt_policy, seconds):
    ...
    return dict(
        feature=semantic_identity(),
        source=source_identity(entry["source"]),
        halts=sorted([list(h) for h in entry["halts"]]),
        halt_policy=halt_policy,
        seconds=seconds,
        quote_initialization_seconds=300,
    )
...
def feature_prefix(identity):
    ...
    return (
        f"derived/tape_features/{I.digest(identity)}/session_date={source['session_date']}"
...
def read_manifest(client, bucket, key):
    ...
    if ident.size_bytes > 2 * 1024**2:
        raise ValueError("manifest exceeds metadata bound")
    ...
    try:
        ...
        body.close()
    if (
        len(payload) != ident.size_bytes
        or hashlib.sha256(payload).hexdigest() != ident.sha256
    ):
...
def verify_object(client, bucket, identity):
    expected = S.ObjectIdentity(
        **{k: identity[k] for k in ("object_key", "sha256", "size_bytes", "rows")}
    )
...
def recover_features(client, bucket, identity):
    ...
    if meta is None:
        return None
    if meta["partition_identity"] != identity:
        raise ValueError("feature semantic identity mismatch")
    if (
        meta["counts"]["rows"] != identity["seconds"]
        or meta["verification"]["rows_verified"] != identity["seconds"]
    ):
...
def crop(source, target, end):
    ...
    with pq.ParquetWriter(target, pf.schema_arrow, compression="zstd") as out:
        for batch in pf.iter_batches(batch_size=25000, use_threads=False):
            ...
            keep = table.filter(pc.less(table["sip_timestamp"], end))
            if keep.num_rows:
                out.write_table(keep)
    ...
    *,
    ...
    if existing is not None:
        ...
        if summary_contract is not None:
            result["summary"] = materialize_summary(
                client, bucket, identity, summary_contract, summarizer, scratch
            )
        return result
    source = entry["source"]
    key = feature_prefix(identity)
    for stream in ("quotes", "trades"):
        verify_object(client, bucket, source[stream])
    # One staged pair and (probe only) a bounded prefix. Keep 3 GiB disk headroom.
    import shutil

    if (
        shutil.disk_usage(scratch).free
        < 2 * sum(source[k]["size_bytes"] for k in ("quotes", "trades")) + 3 * 1024**3
    ):
        raise ValueError("insufficient scratch headroom")
    with S.staged_symbol_day(
        client,
        bucket,
        source["session_date"],
        source["symbol"],
        ...
        for stream in ("quotes", "trades"):
            ...
            S.validate_identities(
                expected, staged_identities[expected.object_key], require_rows=True
            )
        with tempfile.TemporaryDirectory(dir=scratch) as work:
            work = Path(work)
            quotes, trades = staged.quotes, staged.trades
            if seconds != 57600:
                end = F.V.session_bounds(source["session_date"])[0] + seconds * F.V.NS
                for stream in ("quotes", "trades"):
                    crop(getattr(staged, stream), work / (stream + ".parquet"), end)
                quotes, trades = work / "quotes.parquet", work / "trades.parquet"
            path = work / "features.parquet"
            tick = time.monotonic()
            meta = F.write(
                quotes,
                trades,
                ...
                dict(
                    source=identity["source"],
                    config_hash=I.digest(identity),
                    halt_policy=halt_policy,
                ),
                halts=entry["halts"],
                seconds=seconds,
            )
            meta["verification"] = F.verify(
                path, source["session_date"], source["symbol"], expected_rows=seconds
            )
            if include_distributions:
                meta["distributions"] = F.distributions(path)
            ...
            S.publish_parquet_immutable(
                client, bucket, path, key + "/features.parquet", upload_workers=1
            )
            I.save(work / "manifest.json", meta)
            S.publish_file_immutable(
                client,
                bucket,
                work / "manifest.json",
                key + "/manifest.json",
            ...
            if recover_features(client, bucket, identity) is None:
                raise ValueError("feature recovery failed")
...
def recover_summary(client, bucket, identity, contract):
    ...
    if meta is None:
        return None
    if meta["feature_identity"] != identity or meta["summary_contract"] != contract:
...
def materialize_summary(
    client, bucket, identity, contract, summarizer, scratch, *, local_feature=None
):
    # Caller cannot summarize an uncommitted feature object.
    feature = recover_features(client, bucket, identity)
    if feature is None:
        raise ValueError("features not committed")
    ...
    with tempfile.TemporaryDirectory(dir=scratch) as work:
        work = Path(work)
        source = work / "features.parquet"
        target = work / "retrieval.parquet"
        if local_feature is None:
            S.download_verified_parquet(
                client,
                bucket,
                feature_prefix(identity) + "/features.parquet",
                source,
                transfer_workers=1,
            )
        else:
            source = Path(local_feature)
            if S.sha256_file(source) != feature["sha256"]:
                raise ValueError("local feature identity mismatch")
        tick = time.monotonic()
        rows = summarizer(source, target)
        elapsed = time.monotonic() - tick
        artifact = S.publish_parquet_immutable(
            client, bucket, target, key + "/retrieval.parquet", upload_workers=1
        )
        meta = dict(
            ...
            implementation_sha256=S.sha256_file(Path(summarizer.__code__.co_filename)),
            retrieval_identity={
                k: artifact[k] for k in ("object_key", "size_bytes", "sha256", "rows")
            },
        )
        I.save(work / "manifest.json", meta)
        S.publish_file_immutable(
        ...
        if recover_summary(client, bucket, identity, contract) is None:
            raise ValueError("summary recovery failed")
    return meta
