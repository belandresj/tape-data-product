...
from tape_data_product.features.all_feature_month_schema import (
    ...
    REGISTRY,
    field_metadata,
    schema as explicit_schema,
    NUMERICAL_POLICY,
)
from tape_data_product.features.all_feature_month_moments import Moments

AGE_VERSION = "exact_midpoint_change_age_status_reset_v2"
NS = V.NS
BASE_DIAGNOSTICS = (
    "state_observed_second_count",
    "state_mature",
    "movement_valid_5s_count",
    "movement_valid_1s_count",
    "movement_support_valid",
    "activity_valid_second_count",
    "activity_support_valid",
    "quote_age_observation_count",
    "trade_age_observation_count",
    "quoted_spread_valid",
    "quoted_spread_valid_fraction",
    "state_fully_post_halt",
    "state_pre_halt_observation_fraction",
    "state_contains_pre_halt_history",
    "state_post_halt_observed_seconds",
    "state_carried_forward_during_halt",
)
...
def equal(a, b, label):
    ...
    if (
        not finite(a)
        or not finite(b)
        or not math.isclose(a, b, rel_tol=1e-10, abs_tol=1e-12)
    ):
...
def rows(
    ...
    if columns and set(columns) - set(pf.schema_arrow.names):
        raise ValueError(
            "missing required published metadata: "
            + str(sorted(set(columns) - set(pf.schema_arrow.names)))
        )
        ...
        if validate_finite:
            for field, column in zip(selected.schema, selected.columns):
                if (
                    ...
                    and not pc.all(pc.fill_null(pc.is_finite(column), True)).as_py()
                ):
...
class Movement:
    ...
    def push(self, row):
        ...
        if not active:
            self.slots += 1
            self.ends.append(
                (row["midpoint"], row["primitive_quote_source_file_accepted"])
            )
        ...
        result = dict(
            movement_5s_bps=five,
            movement_5s_valid=finite(five),
            movement_1s_valid=finite(one),
            halt_resume_boundary=resume,
            halt_generation=self.generation,
        )
        for h in HORIZONS:
            w = self.windows[h]
            if (
                row[f"movement_valid_5s_count_{h}s"] != w.count
                or row[f"movement_valid_1s_count_{h}s"] != self.ones[h].count
            ):
            ...
            expected_support = (
                ...
                and w.count >= math.ceil(0.8 * (h - 4))
                and self.ones[h].count >= math.ceil(0.8 * h)
            )
            if (
                row[f"state_mature_{h}s"] != (count == h)
                or row[f"movement_support_valid_{h}s"] != expected_support
            ):
                raise ValueError(
                    "movement support/maturity gate reconstruction mismatch"
                )
            equal(
                w.mean(),
                (
                    None
                    if row[f"midpoint_movement_bps_per_30s_{h}s"] is None
                    else row[f"midpoint_movement_bps_per_30s_{h}s"] / 6
                ),
                "movement mean",
            )
            result[f"movement_participation_{h}s"] = (
                w.participation() if count == h and expected_support else math.nan
            )
...
class MidpointAge:
    ...
    def __init__(self, events, start, check=lambda: None):
        self.cursor = V.Cursor(events)
        self.start = start
        self.check = check
        self.previous_end = self.epoch = None
        self.was_halt = False
        ...
        while self.cursor.current and self.cursor.current[0] < start:
            ...
            if ts >= start - 300 * NS:
                self.current = event
                self.latest_quote = ts
                self.mid = event["midpoint"] if event["price_state_valid"] else None
        if self.mid is not None:
            self.origin = start

    def pop(self):
        if self.events_consumed % 4096 == 0:
            self.check()
        self.events_consumed += 1
        return self.cursor.pop()

    def reset(self):
        self.current = self.latest_quote = self.mid = self.last_change = self.origin = (
            None
        )
        self.windows = {h: V.RollingValues(h) for h in HORIZONS}
        self.elapsed = 0
    ...
    def begin_interval(self, row):
        ...
        left = t - NS
        active = row["halt_interval_active"]
        gap = self.previous_end is not None and (
            t != self.previous_end + NS or epoch != self.epoch
        )
        if self.previous_end is not None and t <= self.previous_end:
            raise ValueError("duplicate or unordered age clock")
        if gap or active or self.was_halt:
            self.reset()
        if (
            self.source_lost
            and row["primitive_quote_source_file_accepted"]
            and self.current
            and self.current["price_state_valid"]
        ):
            self.mid = self.current["midpoint"]
            self.origin = left
    ...
    def begin_integrals(self, row):
        ...
        self.spread_mass = 0.0
        self.midpoint_integral = V.MidpointIntegral()
        self.last = row["interval_end_ns"] - NS

    def integrate(self, a, b):
        if self.current is None or self.interval_row["halt_interval_active"] or b <= a:
            return
        if self.current["price_state_valid"]:
            ...
            if not self.current.get("locked", False):
                self.spread_duration += dt
                self.spread_mass += (dt / NS) * self.current.get("spread_bps", 0.0)
    ...
    def consume_event(self, ts, event):
        ...
        if row["halt_interval_active"] or ts < left:
            return
        self.integrate(self.last, ts)
        self.last = ts
        valid = (
            event["price_state_valid"]
            and finite(event["midpoint"])
            and event["midpoint"] > 0
        )
        if not valid:
    ...
    def end_interval(self, row):
        ...
        duration, spread_duration, mass, spread_mass = (
            self.duration,
            self.spread_duration,
            self.midpoint_integral.mass(),
            self.spread_mass,
        )
        if (
            not 0 <= spread_duration <= duration <= NS
            or not finite(mass)
            or not finite(spread_mass)
            or mass < 0
            or spread_mass < 0
        ):
        ...
        if "midpoint" in row:
            equal(reconstructed, row["midpoint"], "quote midpoint")
        accepted = row["primitive_quote_source_file_accepted"]
        raw_age = (
            (t - self.latest_quote) / 1e6
            if self.latest_quote is not None
            and self.current
            and self.current["price_state_valid"]
            else None
        )
        if "primitive_quote_age" in row:
            equal(raw_age, row["primitive_quote_age"], "quote endpoint age")
        if not accepted:
            self.mid = self.last_change = self.origin = None
        self.source_lost = not accepted
        observable = accepted and not active and self.mid is not None
        age = (
            (t - self.last_change) / NS
            if observable and self.last_change is not None
            else math.nan
        )
        status = (
            "known"
            if finite(age)
            else "no_change_observed" if observable else "unobservable"
        )
        if not active:
            self.elapsed += 1
            for w in self.windows.values():
                w.append(age)
        out = dict(
            midpoint_change_age_end_seconds=age,
            midpoint_age_observation_status=status,
            midpoint_observation_start_ns=self.origin if observable else None,
            midpoint_no_change_observed_seconds=(
                (t - self.origin) / NS if status == "no_change_observed" else None
            ),
            midpoint_valid_duration_ns=duration,
            quoted_spread_integral_bps_seconds=spread_mass,
            quoted_spread_valid_duration_ns=spread_duration,
        )
        for h, w in self.windows.items():
            mature = self.elapsed >= h and not active
            supported = w.count >= math.ceil(0.8 * h)
            ...
            out[f"midpoint_change_age_p90_seconds_{h}s"] = (
                w.quantile(0.9) if mature and supported and accepted else math.nan
            )
...
FEATURES_BY_HORIZON = {
    h: tuple(f for f in FEATURES if f.endswith(f"_{h}s")) for h in HORIZONS
}
FAMILIES = tuple((name, v[3]) for name, v in REGISTRY.items() if v[3] != "ratio")
SUPPORT_STEMS = {
    "movement": "movement_support_valid",
    "spread": "quoted_spread_valid",
    "activity": "activity_support_valid",
    "mid_age": "midpoint_change_age_support_valid",
}
...
def quality(row, h, family, value):
    ...
    source = row[
        (
            "primitive_trade_source_file_accepted"
            if family in ("activity", "trade_age")
            else "primitive_quote_source_file_accepted"
        )
    ]
    mature = (
        row[f"midpoint_change_age_mature_{h}s"]
        if family == "mid_age"
        else row[f"state_mature_{h}s"]
    )
    if family in ("trade_age", "quote_age"):
        support = row[f"{family}_observation_count_{h}s"] >= math.ceil(0.8 * h)
    else:
        stem = SUPPORT_STEMS[family]
        support = row[f"{stem}_{h}s"]
    return (
        (REASONS["undefined"] if not finite(value) else 0)
...
def assemble(base, extension, discovery):
    ...
    row.update(
        {f"{x}_{h}s": base[f"{x}_{h}s"] for h in HORIZONS for x in BASE_DIAGNOSTICS}
    )
    row.update(extension)
    count = base["primitive_trade_count"]
    dollars = base["primitive_dollars"]
    if (
        not finite(count)
        or count < 0
        or count >= 2**63
        or int(count) != count
        or not finite(dollars)
        or dollars < 0
    ):
        raise ValueError("invalid transaction primitive")
    row["trade_count_1s"] = int(count)
    row["dollar_volume_1s"] = dollars
    for kind in ("trade", "quote"):
        value = base[f"primitive_{kind}_age"]
        row[f"{kind}_age_end_seconds"] = (
            value / 1000
            if finite(value)
            and not row["halt_interval_active"]
            and row[f"primitive_{kind}_source_file_accepted"]
            else None
        )
    ...
    row["post_discovery_eligible"] = bool(
        discovery.get("verified")
        and discovery.get("endpoint_ns") is not None
        and row["interval_end_ns"] >= discovery["endpoint_ns"]
    )
    left = (row["interval_end_ns"] - session_start(row["session_date"])) // NS - 1
    row["session_segment"] = (
        "premarket" if left < 19800 else "rth" if left < 43200 else "after_hours"
    )
    for h in HORIZONS:
        for target, (source, scale, unit, family) in MAPPINGS.items():
            ...
            row[f"{target}_{h}s"] = (
                value / 6
                if target == "movement_mean_5s_bps" and finite(value)
                else (value / 1000 if "age_p90" in target and finite(value) else value)
            )
        ...
        for stat in ("mean",):
            ...
            value = (
                row[numerator] / spread
                if finite(row[numerator]) and finite(spread) and spread > 0
                else None
            )
            row[f] = value
            reason = (
                row[numerator + "_reason_mask"]
                | row[f"quoted_spread_mean_bps_{h}s_reason_mask"]
            )
        ...
        equal(
            row[f"movement_mean_to_spread_{h}s"],
            published / 6 if finite(published) else None,
            "mean ratio",
        )
        for f in FEATURES_BY_HORIZON[h]:
            carried = not row[f"state_fully_post_halt_{h}s"] and not f.startswith(
                "midpoint_change_age"
            )
            if carried:
                row[f + "_reason_mask"] |= REASONS["carried_history"]
            row[f + "_eda_eligible"] = bool(
                row[f + "_analysis_valid"]
                and not carried
                and row["post_discovery_eligible"]
            )
    return {
...
def extension_columns():
    return (
        ...
        + [
            f"{name}_{h}s"
            for h in HORIZONS
            for name in ("movement_participation", "movement_zero_total")
        ]
        + [
            "midpoint_change_age_end_seconds",
            "midpoint_age_observation_status",
        ...
        + [
            ...
            for h in HORIZONS
            for name in (
                "midpoint_change_age_observation_count",
                "midpoint_change_age_mature",
                "midpoint_change_age_support_valid",
...
def final_columns():
    return list(
        dict.fromkeys(
            [
                *CONTEXT,
                *[f"{x}_{h}s" for h in HORIZONS for x in BASE_DIAGNOSTICS],
                *extension_columns(),
                "trade_count_1s",
                "dollar_volume_1s",
                "trade_age_end_seconds",
                "quote_age_end_seconds",
                ...
                *[
                    f + s
                    for f in FEATURES
                    for s in ("", "_reason_mask", "_analysis_valid", "_eda_eligible")
                ],
            ]
        )
    )
...
def extension_rows(
    ...
    *,
    ...
    movement = Movement()
    age = MidpointAge(
        V.events(quotes, "quote", day, batch_size=batch_size, stats=stats),
        V.session_bounds(day)[0],
        check,
    )
    intervals = iter(halts or ())
    halt = next(intervals, None)
    for base in rows(base_path, EXTENSION_BASE_COLUMNS, batch_size, check, limit):
        if base["session_date"] != day or base["symbol"] != symbol:
            raise ValueError("base partition key mismatch")
        if halts is not None:
            right = base["interval_end_ns"]
            left = right - NS
            while halt is not None and halt[1] <= left:
                halt = next(intervals, None)
            active = bool(halt is not None and halt[0] < right and halt[1] > left)
            if base["halt_interval_active"] != active:
                raise ValueError(
                    "base rows disagree with frozen effective halt overlay"
                )
...
def write_rows(path, iterator, metadata, *, check=lambda: None, columns=None):
    ...
    if partial.exists():
        raise FileExistsError(
            "private partial requires explicit discard/restart: " + str(partial)
        )
    buffer = []
    writer = None
    count = 0
    ...
    if footer.metadata.num_rows != count or footer.schema_arrow != schema:
        raise ValueError("output footer mismatch")
...
def joined_rows(
    base, extension, discovery, batch_size=4096, check=lambda: None, limit=None
):
    for a, b in zip_longest(
        rows(base, BASE_COLUMNS, batch_size, check, limit),
        rows(extension, batch_size=batch_size, check=check),
    ):
...
def verify(path, day, symbol, expected_rows, check=lambda: None, trace_ends=()):
    from tape_data_product.features.all_feature_month_verify import (
