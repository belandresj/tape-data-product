...
import time
import uuid
from tape_data_product.features import all_feature_month_selection as SELECTION
...
def client_for(queue):
    ...
    from tape_data_product.storage.tq_cost_guard import CostGuard, load_policy

    plan = read(Path(queue) / "plan.json")
    policy = (
        load_policy(ROOT / plan["cost_policy_path"])
        if "cost_policy_path" in plan
        else plan["cost_policy"]
    )
    if plan.get("cost_policy") != policy:
        raise ValueError("shared request policy identity changed")
    guard = CostGuard(Path(plan["cost_ledger"]), policy)
    cfg = S.load_r2_settings()
    client = S.build_client(cfg)
...
def inventory(selection, output):
    ...
    db.execute(
        "CREATE TABLE members(day TEXT,symbol TEXT,status TEXT,reason TEXT,payload TEXT,PRIMARY KEY(day,symbol))"
    )
    counts = Counter()
    try:
        for member in source.execute("SELECT * FROM members ORDER BY day,symbol"):
            check_yield()
            try:
                payload = remote_entry(member, client, bucket)
                status = "admitted"
                reason = None
            except (ValueError, KeyError) as exc:
                payload = dict(
                    discovery=json.loads(member["discovery"]),
                    frozen_entry=member["entry"],
                    frozen_receipt=member["receipt"],
                )
                status = "excluded"
                reason = str(exc)
            db.execute(
                "INSERT INTO members VALUES (?,?,?,?,?)",
                (member["day"], member["symbol"], status, reason, json.dumps(payload)),
            )
            db.commit()
            counts[status] += 1
    finally:
        ...
        db.close()
    meta = dict(
        version="tape_eda_verified_inventory_v1",
        month=selected["month"],
        selection_hash=selected["selection_hash"],
        selection_root=str(Path(selection).resolve()),
        inventory_sha256=sha(output / "inventory.sqlite"),
        counts=dict(counts),
        requests=dict(requests),
        code_identity=code_identity(),
        inventory_code_identity=inventory_identity(),
        queue=selected["queue"],
        coverage_claim="verified available inputs only; exclusions and aggregate date coverage remain explicit",
    )
    meta["inventory_hash"] = digest(meta)
    save(output / "inventory_manifest.json", meta)
    return meta


def verified_inventory(root):
    root = Path(root)
    meta = read(root / "inventory_manifest.json")
    expected = meta.pop("inventory_hash")
    if (
        digest(meta) != expected
        or sha(root / "inventory.sqlite") != meta["inventory_sha256"]
    ):
        raise ValueError("frozen inventory identity changed")
    selected = verify_selection(meta["selection_root"])
    if selected["selection_hash"] != meta["selection_hash"]:
    ...
    if any(recorded.get(k) != v for k, v in current.items()):
        raise ValueError(
            "inventory verification implementation changed; renew inventory"
        )
    meta["inventory_hash"] = expected
    return meta
...
def local_identity(path, expected):
    ...
    if (
        sha(path) != expected["sha256"]
        or Path(path).stat().st_size != expected["size_bytes"]
        or pq.ParquetFile(path).metadata.num_rows != expected["rows"]
    ):
...
def partition(
    ...
    *,
    ...
    if not 1 <= seconds <= 57600 or not 1 <= batch_size <= 25000:
    ...
    if (work / "manifest.json").exists():
        ...
        if (
            prior["input_hash"] != digest(payload)
            or prior["code_identity"] != code_identity()
            or prior["seconds"] != seconds
            or prior.get("execution_selection_hash") != execution_selection_hash
        ):
            raise ValueError("resume identity changed")
        C.verify(
            work / "features.parquet",
            payload["entry"]["source"]["session_date"],
            payload["entry"]["source"]["symbol"],
            seconds,
            check_yield,
        )
        if sha(work / "features.parquet") != prior["output"]["sha256"]:
            raise ValueError("completed output identity changed")
        return prior
    if any(work.iterdir()):
        raise ValueError(
            "private incomplete partition requires reviewed restart in a new attempt directory"
        )
    check_yield()
    ...
    attempt_identity = dict(
        ...
        code_identity=code_identity(),
        seconds=seconds,
        execution_selection_hash=execution_selection_hash,
        state="staging",
    )
    save(work / "attempt.json", attempt_identity)
    source = payload["entry"]["source"]
    day, symbol = source["session_date"], source["symbol"]
    source_paths = {}
    source_objects = {
        "quotes": source["quotes"],
        "base": dict(
            object_key=payload["base_key"],
            sha256=payload["base"]["sha256"],
            size_bytes=payload["base"]["bytes"],
            rows=57600 if local is None else payload["base"]["counts"]["rows"],
        ),
    }
    needed = sum(x["size_bytes"] for x in source_objects.values()) * 4
    if shutil.disk_usage(work).free < 3 * GiB + needed:
        raise ResourceWait("insufficient staged partition disk headroom")
    for name, expected in source_objects.items():
        ...
        if local:
            ...
            actual = STORE.S.download_verified_parquet(
                client, bucket, expected["object_key"], path, transfer_workers=1
            )
            STORE.S.validate_identities(
                STORE.S.ObjectIdentity(
                    **{
                        k: expected[k]
                        for k in ("object_key", "sha256", "size_bytes", "rows")
                    }
                ),
                actual,
                require_rows=True,
    ...
    if not local:
        ...
        if (
            ...
            or provenance["config_hash"] != digest(identity)
            or provenance["halt_policy"] != identity["halt_policy"]
        ):
        ...
        if (
            published["contract"] != payload["base"]["contract"]
            or published["contract"] != expected
        ):
            raise ValueError(
                "embedded base implementation identity differs from verified current engine"
            )
    ...
    identity = dict(
        version=C.VERSION,
        age_version=C.AGE_VERSION,
        execution_selection_hash=execution_selection_hash,
        input_hash=digest(payload),
        code_identity=code_identity(),
        source=source,
        base_sha256=payload["base"]["sha256"],
        halts=payload["entry"]["halts"],
        effective_halt_identity=payload["base"].get("partition_identity"),
        seconds=seconds,
        batch_size=batch_size,
        output_batch_size=1024,
        quantile="linear",
        movement_mean_tolerance=dict(relative=1e-10, absolute=1e-12),
        mappings=C.MAPPINGS,
        fields=C.field_metadata(),
        reason_bits=C.REASONS,
        full_session=seconds == 57600,
        capability=capability,
        numerical_policy=C.NUMERICAL_POLICY,
    )
    ext = work / "extension.parquet"
    raw_stats = Counter()
    extension_started = time.monotonic()
    ext_result = C.write_rows(
        ext,
        C.extension_rows(
            base,
            source_paths["quotes"],
    ...
    save(
        work / "attempt.json",
        attempt_identity | {"state": "extension_written", "extension": ext_result},
    )
    footer = pq.ParquetFile(ext)
    if (
        footer.metadata.num_rows != seconds
        or footer.schema_arrow.names != C.extension_columns()
    ):
        raise ValueError("extension footer/schema mismatch")
    ...
    result = C.write_rows(
        final,
        C.joined_rows(
            base, ext, payload["discovery"], batch_size, check_yield, limit=seconds
        ),
        identity | {"extension": ext_result},
        check=check_yield,
        columns=C.final_columns(),
    )
    ...
    coverage = verification.pop("coverage")
    traces = verification.pop("traces")
    if result["rows"] != seconds or result["sha256"] != verification["sha256"]:
    ...
    measurement = dict(
        **identity,
        ...
        assembly_verify_seconds=time.monotonic() - assembly_started,
        staged_bytes=sum(x["size_bytes"] for x in source_objects.values()),
        raw_quote_rows=source["quotes"]["rows"],
        raw_stats=dict(raw_stats),
        extension_seconds=extension_seconds,
        scratch_bytes=sum(p.stat().st_size for p in work.iterdir() if p.is_file()),
    )
    save(work / "coverage.json", coverage)
    measurement["coverage_seconds"] = (
        0.0  # Included in the single stored verification pass.
    )
    measurement["coverage_sha256"] = sha(work / "coverage.json")
    measurement["io_passes"] = dict(
        base_decodes=2,
        quote_decodes=1,
        extension_decodes=1,
        final_decodes=1,
        base_projected_columns=C.BASE_COLUMNS,
        extension_base_projected_columns=C.EXTENSION_BASE_COLUMNS,
        extension_columns=C.extension_columns(),
        final_columns=C.final_columns(),
        base_rows_per_pass=seconds,
        quote_projected_columns=list(C.V.MARKET.QUOTE_COLUMNS),
        base_compressed_bytes=payload["base"]["bytes"],
        quote_compressed_bytes=source["quotes"]["size_bytes"],
        extension_compressed_bytes=ext_result["bytes"],
        final_compressed_bytes=result["bytes"],
        hashing="bounded byte streams, additional to decodes",
        footer_reads="staging and each completed write",
    )
    save(work / "validation_traces.json", traces)
    save(
        work / "attempt.json",
        attempt_identity | {"state": "verified", "output": result},
    )
    check_yield()
    save(work / "manifest.json", measurement)
    return measurement
...
def _partition(args):
    ...
    if (
        args.seconds == 57600
        and os.environ.get("TAPE_EDA_FULL_APPROVED") != meta["inventory_hash"]
    ):
        raise ValueError("full symbol-day worker requires approved parent run")
    db = connect(Path(args.inventory) / "inventory.sqlite", True)
    row = db.execute(
        "SELECT payload FROM members WHERE day=? AND symbol=? AND status='admitted'",
        (args.day, args.symbol),
    ).fetchone()
    db.close()
    if row is None:
        raise ValueError("partition is not in frozen admitted inventory")
    selection_hash = None
    if args.execution_selection:
        chosen = SELECTION.verify(args.execution_selection, meta["inventory_hash"])
        selection_hash = chosen["selection_hash"]
        selected = connect(args.execution_selection / "selection.sqlite", True)
        member = selected.execute(
            "SELECT seconds FROM members WHERE day=? AND symbol=?",
            (args.day, args.symbol),
        ).fetchone()
        selected.close()
        if member is None or member["seconds"] != args.seconds:
            raise ValueError("worker selection membership/grid mismatch")
    ...
    result = partition(
        ...
        client=client,
        bucket=bucket,
        execution_selection_hash=selection_hash,
        requests=requests,
    )
...
def synthetic(args):
    ...
    manifest = F.write(
        q, t, day, "TEST", base, {}, halts=halts, seconds=seconds, batch_size=4096
    )

    def obj(path):
        return dict(
            object_key="synthetic/" + path.name,
            sha256=sha(path),
            size_bytes=path.stat().st_size,
            rows=C.pq.ParquetFile(path).metadata.num_rows,
        )

    source = dict(session_date=day, symbol="TEST", quotes=obj(q), trades=obj(t))
    payload = dict(
        entry=dict(source=source, halts=halts),
        base=manifest,
        base_key="synthetic/base.parquet",
        discovery=dict(
            verified=True,
            endpoint_ns=start + 100 * C.NS,
            received_at_ns=None,
            timing_basis="synthetic nominal completion",
            provenance_hash="synthetic",
        ),
    )
    result = partition(
        payload, root / "partition", seconds=seconds, local=dict(quotes=q, base=base)
    )
    save(
        root / "result.json",
        dict(
            ...
            trade_events=1,
            result=result,
            kind="synthetic_production_bootstrap",
        ),
    )
...
def choose_probes(db):
    ...
    db.execute(
        "CREATE TEMP TABLE sizes(day TEXT,symbol TEXT,bytes INTEGER,halted INTEGER)"
    )
    for row in db.execute(
        "SELECT day,symbol,payload FROM members WHERE status='admitted'"
    ):
        ...
        db.execute(
            ...
            (
                row["day"],
                row["symbol"],
                p["entry"]["source"]["quotes"]["size_bytes"],
                bool(p["entry"]["halts"]),
            ),
        )
    result = []
    for label, order, where in [
        ("sparse", "bytes ASC", ""),
        ("dense", "bytes DESC", ""),
        ("halt_boundary", "bytes ASC", "WHERE halted=1"),
    ]:
        row = db.execute(
            f"SELECT day,symbol FROM sizes {where} ORDER BY {order},day,symbol LIMIT 1"
        ).fetchone()
        if row:
            result.append(dict(label=label, day=row["day"], symbol=row["symbol"]))
    return result
...
def release_staged_copies(work):
    ...
    if not (work / "manifest.json").exists():
        raise ValueError("cannot clean an incomplete attempt")
    for name in (
        "quotes.parquet",
        "base.parquet",
        "base_prefix.parquet",
...
def probe(args):
    meta = verified_inventory(args.inventory)
    selection = (
        SELECTION.verify(args.execution_selection, meta["inventory_hash"])
        if args.execution_selection
        else None
    )
    bootstrap = read(args.bootstrap)
    if (
        bootstrap.get("returncode") != 0
        or not bootstrap.get("target_met")
        or bootstrap.get("stop_reason")
    ):
    ...
    if (
        ...
        or read(result_path)["code_identity"] != code_identity()
    ):
        raise ValueError("bootstrap code identity changed")
    output = Path(args.output)
    output.mkdir(parents=True, exist_ok=False)
    db = connect(Path(args.inventory) / "inventory.sqlite", True)
    selected = choose_probes(db)
    db.close()
    if {x["label"] for x in selected} != {"sparse", "dense", "halt_boundary"}:
        raise ValueError("representative halt case unavailable; report inventory issue")
    measurements = []
    for choice in selected:
        work = output / choice["label"]
        report = output / (choice["label"] + "_resources.json")
        ...
        payload = json.loads(
            db.execute(
                "SELECT payload FROM members WHERE day=? AND symbol=?",
                (choice["day"], choice["symbol"]),
            ).fetchone()[0]
        )
        db.close()
        ...
        start = (
            int(
                datetime.combine(
                    datetime.fromisoformat(choice["day"]).date(),
                    dt_time(4),
        ...
        if choice["label"] == "dense":
            seconds = max(seconds, 24000)  # Include dense RTH events through 10:40 ET.
        if choice["label"] == "halt_boundary":
            seconds = max(
                seconds,
                math.ceil((payload["entry"]["halts"][0][1] - start) / 10**9) + 301,
            )
        if seconds > 54000:
            raise ValueError(
                "representative prefix would approach a full session; select an earlier independently known case"
            )
        needed = 4 * (
            payload["entry"]["source"]["quotes"]["size_bytes"]
            + payload["base"]["bytes"]
        )
        command = [
            sys.executable,
        ...
        measurement = supervise(
            ...
            scratch=output,
            report=report,
            projected_peak=max(bootstrap["peak_new_rss_bytes"], 192 * MiB),
            needed=needed,
        )
        if (
            measurement["returncode"]
            or measurement["stop_reason"]
            or not measurement["target_met"]
        ):
        ...
        measurements.append(
            dict(
                **choice,
                ...
                partition=part,
                requests=read(work / "requests.json"),
            )
        )
    ...
    for r in SELECTION.members(db, args.execution_selection):
        ...
        admitted += 1
        total_rows += r["seconds"]
        total_quotes += p["entry"]["source"]["quotes"]["rows"]
        total_raw_bytes += p["entry"]["source"]["quotes"]["size_bytes"]
        total_base_bytes += p["base"]["bytes"]
    db.close()
    bytes_per_row = max(
        x["partition"]["output"]["bytes"] / x["seconds"] for x in measurements
    )
    ...
    row_seconds = max(
        (
            x["partition"]["extension_seconds"]
            + x["partition"]["assembly_verify_seconds"]
            + x["partition"]["coverage_seconds"]
        )
        / x["seconds"]
        for x in measurements
    )
    dense = next(x for x in measurements if x["label"] == "dense")
    event_seconds = dense["partition"]["extension_seconds"] / max(
        1, dense["partition"]["raw_stats"].get("quote_rows_consumed", 0)
    )
    bytes_per_second = min(
        x["partition"]["staged_bytes"] / max(0.001, x["partition"]["staging_seconds"])
        for x in measurements
    )
    download_seconds = (total_raw_bytes + total_base_bytes) / bytes_per_second
    runtime_upper = (
        total_rows * row_seconds
        + total_quotes * event_seconds
        + download_seconds
        + admitted * 11
    )
    projected_final = (
        max(total_rows * bytes_per_row * 1.5, total_base_bytes * 1.8)
        + admitted * 128 * 1024
    )
    staging_fit = staging_model(
        measurements, total_raw_bytes + total_base_bytes, admitted
    )
    warm_runtime = (
        total_rows * row_seconds
        + total_quotes * event_seconds
        + staging_fit["projected_seconds"]
        + admitted
        + 10
    )

    checkpoint = dict(
        kind="representative_prefix_checkpoint",
        inventory_hash=meta["inventory_hash"],
        execution_selection_hash=selection["selection_hash"] if selection else None,
        code_identity=code_identity(),
        cases=measurements,
        projected_rows=total_rows,
        projected_final_bytes=math.ceil(projected_final),
        projected_download_bytes=total_raw_bytes + total_base_bytes,
        projected_runtime_seconds_conservative=runtime_upper,
        projected_runtime_seconds_continuous_admission=warm_runtime,
        staging_model=staging_fit,
        projection_components=dict(
            row_seconds=row_seconds,
            event_seconds=event_seconds,
            staged_bytes_per_second=bytes_per_second,
            admission_and_startup_seconds_per_partition=11,
            continuous_admission_initial_seconds=10,
            continuous_admission_startup_allowance_per_partition=1,
            continuous_admission_limitation="fresh 10-second admission if observation gap exceeds one second, pressure, or incumbent changes; startup allowance is an assumption",
            measured_max_output_bytes_per_row=bytes_per_row,
        ...
        projected_requests=admitted
        * max(sum(x["requests"].values()) for x in measurements),
        peak_new_rss_bytes=max(
            x["resources"]["peak_new_rss_bytes"] for x in measurements
        ),
...
def staging_model(measurements, total_bytes, partitions):
    ...
    points = [
        (
            float(x["partition"]["staged_bytes"]),
            float(x["partition"]["staging_seconds"]),
        )
        for x in measurements
    ]
    ...
    slope = (
        max(0.0, sum((x - mx) * (y - my) for x, y in points) / variance)
        if variance
        else 0.0
    )
    ...
    if intercept == 0:
        slope = sum(x * y for x, y in points) / sum(x * x for x, y in points)
    intercept = max(intercept, max(y - slope * x for x, y in points))
    return dict(
        fixed_seconds_per_partition=intercept,
...
def run(args):
    ...
    if checkpoint.get("execution_selection_hash") != selection_hash:
    ...
    if (
        checkpoint["inventory_hash"] != meta["inventory_hash"]
        or checkpoint["code_identity"] != code_identity()
    ):
        raise ValueError("checkpoint inputs/code changed")
    if not args.full_run_approved:
        raise ValueError(
            "full external-data acceptance requires user confirmation after the measured checkpoint"
        )
    output = Path(args.output)
    ...
    with (output / "run.lock").open("a") as lock:
        ...
        identity = dict(
            ...
            execution_selection_hash=selection_hash,
            execution_selection_root=(
                str(args.execution_selection.resolve())
                if args.execution_selection
                else None
            ),
            schema_version="tape_data_product_eda_v2",
            horizons_seconds=[60, 300],
            cadence_seconds=1,
            age_version="exact_midpoint_change_age_reset_halt_continuity_v1",
            quantile="linear",
            ...
            selection_root=meta["selection_root"],
            inventory_root=str(Path(args.inventory).resolve()),
        )
        ...
        if (
            ...
            < 3 * GiB + checkpoint["projected_final_bytes"]
        ):
            raise ResourceWait(
                "volume cannot hold projected dataset plus 3 GiB reserve"
            )
        save(
            manifest_path,
            dict(
        ...
        try:
            for member in SELECTION.members(db, args.execution_selection):
                ...
                if (target / "manifest.json").exists():
                    ...
                    if (
                        prior["input_hash"] != digest(payload)
                        or prior["code_identity"] != code_identity()
                        or prior["seconds"] != member["seconds"]
                        or sha(target / "features.parquet") != prior["output"]["sha256"]
                    ):
                        raise ValueError("completed partition identity changed")
                    continue
                if target.exists():
                    raise ValueError("partial final publication requires inspection")
                ...
                if args.execution_selection:
                    command += ["--execution-selection", str(args.execution_selection)]
                result = supervise(
                    command,
                    research=ROOT / "research",
                    scratch=attempt,
                    report=output
                    / "resources"
                    / f"{member['day']}_{member['symbol']}_{attempt.name}.json",
                    projected_peak=checkpoint["peak_new_rss_bytes"],
                    needed=needed,
                    admission_history=admission_history,
                )
                if result["returncode"] or result["stop_reason"]:
                    raise ResourceWait(
                        "partition stopped; inspect private attempt " + str(attempt)
                    )
            ...
            result = supervise(
                ...
                research=ROOT / "research",
                scratch=output / "summary_scratch",
                report=output / "resources" / "summaries.json",
                projected_peak=checkpoint["peak_new_rss_bytes"],
                needed=64 * MiB,
                timeout=7200,
            )
            if result["returncode"] or result["stop_reason"]:
                raise ResourceWait("summary worker interrupted")
            save(
                manifest_path,
                dict(
                    identity=identity,
                    state=(
                        "complete_selected_preview"
                        if selection
                        else "complete_over_verified_available_inputs"
                    ),
                    coverage_claim=meta["coverage_claim"],
                    excluded_members=meta["counts"].get("excluded", 0),
                ),
            )
            ...
            save(
                ...
                dict(
                    identity=identity,
                    state=(
                        "resource_wait" if isinstance(exc, ResourceWait) else "failed"
                    ),
                    reason=str(exc),
                ),
            )
...
def artifacts(inventory_root, output):
    ...
    output = Path(output)
    db = connect(output / "coverage.sqlite")
    ...
    verified_rows = 0
    db.execute(
        "CREATE TABLE counts(day TEXT,symbol TEXT,session TEXT,feature TEXT,rows INTEGER,valid INTEGER,nulls INTEGER,zeros INTEGER,post INTEGER,eda INTEGER,reason TEXT,histogram TEXT,minimum REAL,maximum REAL,axis TEXT)"
    )
    source = connect(Path(inventory_root) / "inventory.sqlite", True)
    dataset = (
        read(output / "dataset_manifest.json")
        if (output / "dataset_manifest.json").exists()
        else {}
    )
    selection_root = dataset.get("identity", {}).get("execution_selection_root")
    selected = (
        connect(Path(selection_root) / "selection.sqlite", True)
        if selection_root
        else None
    )
    ...
    with pq.ParquetWriter(
        output / "catalog.parquet.partial", catalog_schema, compression="zstd"
    ) as catalog:
        for member in source.execute("SELECT * FROM members ORDER BY day,symbol"):
            payload = json.loads(member["payload"])
            path = (
            ...
            item = dict(
                session_date=member["day"],
                symbol=member["symbol"],
                status=member["status"],
                reason=member["reason"],
                discovery=json.dumps(payload.get("discovery")),
                source_base_identity=json.dumps(payload),
            ...
            chosen = (
                selected is None
                or selected.execute(
                    "SELECT 1 FROM members WHERE day=? AND symbol=?",
                    (member["day"], member["symbol"]),
                ).fetchone()
                is not None
            )
            if not chosen:
                item.update(
                    status="unselected", reason="not in immutable execution selection"
                )
            elif (
                ...
                item.update(
                    status="missing_selected", reason="no verified selected partition"
                )
            elif member["status"] == "admitted":
                part = read(path.parent / "manifest.json")
                item.update(
                    status="complete",
                    output_reference=str(path.relative_to(output)),
                    output_sha256=part["output"]["sha256"],
                    rows=part["output"]["rows"],
                )
                if (
                    part["verification"].get("reconstructed_fields") != list(C.FEATURES)
                    or part["verification"]["rows_verified"] != part["output"]["rows"]
                ):
                    raise ValueError("partition reconstruction evidence missing")
                verified_rows += part["verification"]["rows_verified"]
                coverage_path = path.parent / "coverage.json"
                if sha(coverage_path) != part["coverage_sha256"]:
                    raise ValueError("partition coverage identity changed")
                    ...
                    db.execute(
                        ...
                        (
                            ...
                            *(
                            ...
                            json.dumps(g["reason"]),
                            json.dumps(g["histogram"]),
                            g["minimum"],
                            g["maximum"],
                            json.dumps(g["axis"]),
                        ),
                    )
                    if sum(g["histogram"]) != g["eda"]:
                        raise ValueError("summary mass inconsistent")
                    db.executemany(
                        "INSERT INTO bins VALUES (?,?,?,?)",
                        (
                            (g["session_segment"], g["feature"], i, mass)
                            for i, mass in enumerate(g["histogram"])
                        ),
                    )
                db.commit()
    ...
    schema = pa.schema(
        [
            pa.field(x, pa.string())
            for x in ("session_date", "symbol", "session_segment", "feature")
        ]
        + [
            pa.field(x, pa.int64())
            for x in (
        ...
        + [
            pa.field("reason_counts_json", pa.string()),
            pa.field("histogram_json", pa.string()),
            pa.field("minimum", pa.float64()),
            pa.field("maximum", pa.float64()),
            pa.field("axis_json", pa.string()),
        ]
    )
    with pq.ParquetWriter(
        output / "coverage.parquet.partial", schema, compression="zstd"
    ) as writer:
        cursor = db.execute("SELECT * FROM counts ORDER BY day,symbol,session,feature")
        while batch := cursor.fetchmany(1024):
            writer.write_table(
                pa.Table.from_pylist(
                    [dict(zip(schema.names, tuple(r))) for r in batch], schema=schema
                )
            )
    os.replace(output / "coverage.parquet.partial", output / "coverage.parquet")
    totals = []
    for r in db.execute(
        """SELECT session,feature,SUM(rows),SUM(valid),SUM(nulls),SUM(zeros),SUM(post),SUM(eda),
            ...
            dict(
                zip(
                    (
                        ...
                        "contributing_dates",
                        "eda_contributing_symbols",
                        "eda_contributing_dates",
                    ),
                    tuple(r),
                )
            )
    ...
    if totals:
        pq.write_table(
            pa.Table.from_pylist(totals),
            output / "coverage_totals.parquet",
            compression="zstd",
        )
    hist_schema = pa.schema(
        [
            pa.field("session_segment", pa.string()),
            pa.field("feature", pa.string()),
            pa.field("bin", pa.int64()),
            pa.field("eligible_seconds", pa.int64()),
        ]
    )
    with pq.ParquetWriter(
        output / "histogram_totals.parquet", hist_schema, compression="zstd"
    ) as writer:
        cursor = db.execute(
            "SELECT session,feature,bin,SUM(mass) FROM bins GROUP BY session,feature,bin ORDER BY session,feature,bin"
        )
        while batch := cursor.fetchmany(1024):
            writer.write_table(
                pa.Table.from_pylist(
                    [dict(zip(hist_schema.names, tuple(r))) for r in batch],
                    schema=hist_schema,
                )
            )
    db.close()
...
def main():
    ...
    p.add_argument("--output", type=Path, required=True)
    p = sub.add_parser("_synthetic")
    p.add_argument("--output", type=Path, required=True)
    p = sub.add_parser("_partition")
    p.add_argument("--inventory", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--day", required=True)
    p.add_argument("--symbol", required=True)
    p.add_argument("--seconds", type=int, default=660)
    p = sub.add_parser("_artifacts")
    p.add_argument("--inventory", type=Path, required=True)
    p.add_argument("--output", type=Path, required=True)
    ...
    p.add_argument("--output", type=Path, required=True)
    p.add_argument("--full-run-approved", action="store_true")
    p.add_argument("--resume-reviewed", action="store_true")
    for name in ("probe", "run", "_partition"):
        sub.choices[name].add_argument(
            "--execution-selection",
            type=Path,
            help="immutable preview/full membership, independently hashed",
        )
    ...
    p.add_argument(
        ...
        type=Path,
        required=True,
        help="JSONL: day, symbol, seconds (default 57600)",
    )
    ...
    try:
        if args.command == "freeze":
            result = freeze(args.universe, args.queue, args.output, args.month)
        elif args.command == "select":
            result = SELECTION.create(
                args.inventory, args.output, args.members, args.rule, args.seed
            )
            ...
            result = supervise(
                ...
                research=ROOT / "research",
                scratch=args.output.parent,
                report=args.output.parent / (args.output.name + "_resources.json"),
                projected_peak=320 * MiB,
                needed=64 * MiB,
                timeout=7200,
            )
        elif args.command == "_inventory":
            worker_setup()
            result = inventory(args.selection, args.output)
        elif args.command == "_partition":
            result = _partition(args)
        elif args.command == "_synthetic":
            result = synthetic(args)
        elif args.command == "_artifacts":
            worker_setup()
            result = artifacts(args.inventory, args.output)
        ...
        if (
            ...
            and result.get("kind") == "representative_prefix_checkpoint"
        ):
            print(
                json.dumps(
                    {
                        ...
                        if k not in ("cases", "code_identity")
                    },
                    default=str,
                )
            )
        ...
        print(json.dumps(dict(state="resource_wait", reason=str(exc))))
        sys.exit(75)
