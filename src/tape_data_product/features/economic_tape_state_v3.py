...
from tape_data_product.features import build_market_state as MARKET
from tape_data_product.features import build_rolling_tape_state_v2 as V2
...
FAMILY_BASES = {
    ...
    "displayed_capacity": (
        "mean_displayed_bid_notional",
        "mean_displayed_ask_notional",
    ),
}
FAMILIES = {
    f: tuple(f"{b}_{h}s" for b in bases for h in HORIZONS)
...
def session_bounds(day):
    ...
    return tuple(
        int(datetime.combine(d, time(h), ZoneInfo("America/New_York")).timestamp()) * NS
        for h in (4, 20)
    )
...
def events(path, stream, day, batch_size=25_000, stats=None, *, _include_changes=False):
    ...
    for batch in pf.iter_batches(
        ...
        if stream == "quote":
            ...
            for name in SIGNATURE:
                ...
                if batch.num_rows:
                    if previous_signature is not None:
                        changed[0] = MARKET._scalar_changed(
                            arr[0], previous_signature[name]
                        )
                    if np.issubdtype(arr.dtype, np.floating):
                        changed[1:] = (arr[1:] != arr[:-1]) & ~(
                            np.isnan(arr[1:]) & np.isnan(arr[:-1])
                        )
                    else:
                        changed[1:] = arr[1:] != arr[:-1]
                masks[0] |= changed
            ...
            values = zip(
                *(sem[name].tolist() for name in names[:-3]),
            ...
            if np.any(
                ~np.isin(sem.correction_code, list(MARKET.KNOWN_CORRECTION_CODES))
            ):
                raise ValueError("unaccepted unknown trade correction")
            eligible = np.where(
                (sip >= rth_start) & (sip < rth_end),
        ...
        for ts, sequence, value in zip(sip.tolist(), seq.tolist(), values):
            ...
            if stream == "quote" and not _include_changes:
                for key in ("_changed", "_price_changed", "_size_changed"):
                    del row[key]
            yield ts, row
...
class MidpointIntegral:
    ...
    def add(self, price, duration_ns):
        if (
            ...
            or not 0 <= duration_ns <= NS - self.duration
        ):
    ...
    *,
    ...
    for position in range(seconds):
        ...
        while q.current and q.current[0] < right:
            ...
            if signature is not None:
                if "_changed" in value:
                    ...
                    changed = {
                        name: MARKET._scalar_changed(value[name], signature[name])
                        for name in SIGNATURE
                    }
        ...
        row = dict(
            ...
            quote_source_file_accepted=True,
            trade_source_file_accepted=True,
            midpoint=midpoint_integral.mean(),
            quote_age=(
                (right - latest_q) / 1e6
                if latest_q is not None and current and current["price_state_valid"]
                else math.nan
            ),
...
class RollingValues:
    ...
    def clear(self):
        self.queue = deque()
        self.sorted = []
        self.accumulator = self.compensation = 0.0
        self.nonzero = 0

    def add(self, x):
        total = self.accumulator + x
        self.compensation += (
            (self.accumulator - total) + x
            if abs(self.accumulator) >= abs(x)
    ...
    @property
    def total(self):
        return max(0.0, self.accumulator + self.compensation)
    ...
    def quantile(self, probability):
        ...
        return (
            self.sorted[lo] * (hi - pos) + self.sorted[hi] * (pos - lo)
            if hi != lo
            else self.sorted[lo]
        )
    ...
    def __init__(self, horizon):
        ...
        for name in (
            ...
            setattr(
                self,
                name,
                (RollingValues if name in ("quote_age", "trade_age") else SumValues)(
                    horizon - 4 if name == "move5" else horizon
                ),
            )
...
class FeatureStream:
    ...
    def reset(self):
        ...
        self.churn = {
            h: {
                k: SumValues(h)
                for k in ("changes", "price_changes", "size_changes", "raw_messages")
            }
            for h in HORIZONS
        }
        self.cost = {
            h: {
                k: SumValues(h)
                for k in ("spread_mass", "unlocked_duration", "accepted")
            }
            for h in HORIZONS
        }
        self.quality_duration = {
            h: {k: SumValues(h) for k in ("depth", "midpoint")} for h in HORIZONS
        }
    ...
    def push(self, p):
        ...
        if self.epoch is not None and (
            p["continuity_segment_id"] != self.epoch
            or p["interval_end_ns"] != self.previous_end + NS
        ):
            self.reset()
        ...
        if not active:
            ...
            self.endpoints.append(
                (p["midpoint"], p["quote_source_file_accepted"], p["generation"])
            )
            ...
            def movement(lag):
                ...
                if a[2] != b[2] or not all(x[1] for x in ends[-lag - 1 :]):
                    return math.nan
                return (
                    abs(10_000 * math.log(b[0] / a[0]))
                    if all(math.isfinite(x) and x > 0 for x in (a[0], b[0]))
                    else math.nan
                )
            ...
            for h in HORIZONS:
                w = self.windows[h]
                depth = p["depth_duration"] if p["quote_source_file_accepted"] else 0.0
                for kind in ("depth", "midpoint"):
                    value = p.get(
                        f"{kind}_duration_ns", round(p[f"{kind}_duration"] * NS)
                    )
                    self.quality_duration[h][kind].append(
                        value if p["quote_source_file_accepted"] else 0
                    )
                ...
                w.append(
                    ...
                    move5=move5,
                    move1=move1,
                    activity_valid=p["trade_source_file_accepted"],
                    trade_count=p["trade_count"],
                    dollars=p["dollars"],
                    quote_age=(
                        p["quote_age"] if p["quote_source_file_accepted"] else math.nan
                    ),
                    trade_age=(
                        p["trade_age"] if p["trade_source_file_accepted"] else math.nan
                    ),
                    midpoint_duration=(
                        p["midpoint_duration"]
                        if p["quote_source_file_accepted"]
                        else 0.0
                    ),
                    depth_duration=depth,
                    bid=p["bid_mass"] / depth if depth else math.nan,
                ...
                put(
                    ...
                    and w.move5.count >= math.ceil(0.8 * (h - 4))
                    and w.move1.count >= math.ceil(0.8 * h),
                )
                put("activity_valid_second_count", w.trade_count.count)
                put("activity_support_valid", w.trade_count.count >= math.ceil(0.8 * h))
                ...
                put(
                    "displayed_notional_support_valid",
                    count >= math.ceil(0.8 * h) and depth_total_ns * 5 >= 4 * h * NS,
                )
                put(
                    "mean_displayed_bid_notional",
                    w.bid_weight.total / depth_total if depth_total > 0 else math.nan,
                )
                put(
                    "mean_displayed_ask_notional",
                    w.ask_weight.total / depth_total if depth_total > 0 else math.nan,
                )
                put(
                    "usable_nbbo_fraction",
                    self.quality_duration[h]["midpoint"].total / (h * NS),
                )
                for kind in ("quote", "trade"):
                    age = getattr(w, f"{kind}_age")
                    put(f"{kind}_age_p90", age.quantile(0.9))
                    put(f"{kind}_age_observation_count", age.count)
                pre = (
                    count - w.generation_counts[p["generation"]]
                    if self.post_count is not None
                    else 0
                )
                put(
                    "state_fully_post_halt",
                    self.post_count is None
                    or (count == h and self.post_count >= h + 1),
                )
                ...
                for key, name in (
                    ("changes", "nbbo_state_change"),
                    ("price_changes", "nbbo_price_change"),
                    ("size_changes", "nbbo_size_change"),
                    ("raw_messages", "raw_quote_message"),
                ):
                    field = self.churn[h][key]
                    field.append(
                        p[key] if p["quote_source_file_accepted"] else math.nan
                    )
                    put(f"{name}_rate", field.mean())
                n = self.churn[h]["changes"].count
                put("nbbo_state_change_observation_count", n)
                put("nbbo_state_change_support_valid", n >= math.ceil(0.8 * h))
                c = self.cost[h]
                c["spread_mass"].append(
                    p["spread_mass"] if p["quote_source_file_accepted"] else 0.0
                )
                duration_ns = p.get(
                    "unlocked_duration_ns", round(p["unlocked_duration"] * NS)
                )
                c["unlocked_duration"].append(
                    duration_ns if p["quote_source_file_accepted"] else 0
                )
                ...
                put(
                    ...
                    duration_ns / (h * NS) if structural else math.nan,
                )
                put(
                    "quoted_spread_valid", structural and duration_ns * 10 >= 9 * h * NS
                )
                put(
                    "quoted_spread_bps",
                    (
                        c["spread_mass"].total / duration
                        if structural and duration > 0
                        else math.nan
                    ),
                )
        else:
            for h in HORIZONS:
                ...
                if count == 0:
                    ...
                    for name in (
                        ...
                        "displayed_notional_support_valid",
                        "nbbo_state_change_support_valid",
                    ):
                        out[f"{name}_{h}s"] = False
                    for name in (
                        "displayed_notional_valid_fraction",
                        "usable_nbbo_fraction",
                    ):
                        out[f"{name}_{h}s"] = 0.0
                    for name in (
                        "nbbo_price_change_rate",
                        "nbbo_size_change_rate",
                        "raw_quote_message_rate",
                    ):
                ...
                out[f"state_pre_halt_observation_fraction_{h}s"] = 1.0 if count else 0.0
        ...
        eligible = not active and all(
            out.get(f"{name}_{h}s", False) for h in HORIZONS for name in gates
        )
        eligible &= all(
            out.get(f"{kind}_age_observation_count_{h}s", 0) >= math.ceil(0.8 * h)
            for h in HORIZONS
            for kind in ("quote", "trade")
        )
        eligible &= all(
            math.isfinite(out.get(f, math.nan)) and out[f] >= 0 for f in FEATURES
        )
...
def output_schema():
    ...
    p = dict(
        ...
        halt_interval_id=None,
        historical_ex_post_overlay=True,
        halt_resume_boundary=False,
        quote_source_file_accepted=True,
        trade_source_file_accepted=True,
        midpoint=math.nan,
        quote_age=math.nan,
    ...
    for k, v in row.items():
        t = (
            pa.string()
            if k in ("session_date", "symbol", "halt_interval_id")
            else (
                pa.bool_()
                if isinstance(v, bool)
                else (
                    pa.int64()
                    if isinstance(v, int) or k == "seconds_since_halt_resume"
                    else pa.float64()
                )
            )
        )
...
def write_features(quotes, trades, day, symbol, output, *, identity=None, **kwargs):
    ...
    schema = output_schema().with_metadata(
        {b"v3_identity": json.dumps(identity, sort_keys=True).encode()}
    )
