...
from tape_data_product.features.all_feature_month_inventory import digest, sha, read
...
def code_identity():
    from tape_data_product.features.july_r2_identity import code_identity as current
...
def identity(payload, seconds, inventory_hash, probe_run=None):
    ...
    if probe_run and seconds == 57600:
        raise ValueError("probe namespace cannot contain a full partition")
    result = dict(
        version=VERSION,
        contract="tape_data_product_v1",
        code=code_identity(),
...
def verify_remote(client, bucket, expected):
    S.validate_identities(
        S.ObjectIdentity(**object_id(expected)),
...
def stage(client, bucket, expected, path):
    ...
    S.validate_identities(
        S.ObjectIdentity(**object_id(expected)), actual, require_rows=True
    )


def validate_catalog(catalog):
    body = {k: v for k, v in catalog.items() if k != "catalog_hash"}
    if digest(body) != catalog.get("catalog_hash"):
        raise ValueError("catalog hash mismatch")
    ident = catalog["identity"]
    if ident["version"] != VERSION or ident["contract"] != "tape_data_product_v1":
        raise ValueError("unsupported contract")
    if not 1 <= ident["seconds"] <= 57600 or ident["full_session"] != (
        ident["seconds"] == 57600
    ):
        raise ValueError("invalid partial coverage")
    if (
        catalog["conversions"] != C.field_metadata()
        or catalog["columns"] != C.final_columns()
    ):
        raise ValueError("schema/conversions mismatch")
    if (
        catalog["extension"]["rows"] != ident["seconds"]
        or catalog["base"]["rows"] < ident["seconds"]
    ):
        raise ValueError("catalog row coverage mismatch")
    if catalog["extension"]["object_key"] != prefix(ident) + "/extension.parquet":
        raise ValueError("extension namespace mismatch")


def local_rows(
    catalog, base, extension, *, columns=None, batch_size=4096, check=check_yield
):
    ...
    """
    from tape_data_product.features.july_r2_batches import local_batches

    for batch in local_batches(
        catalog, base, extension, columns=columns, batch_size=batch_size, check=check
    ):
...
def read_batches(
    ...
    *,
    ...
    if not catalog["identity"]["full_session"] and not allow_partial:
        raise ValueError("explicit partial-prefix opt-in required")
    if not 1 <= batch_size <= 25000:
        raise ValueError("batch size must be 1..25000")
    scratch = Path(scratch)
    scratch.mkdir(parents=True, exist_ok=True)
    if shutil.disk_usage(scratch).free < 3 * GiB + sum(
        catalog[k]["size_bytes"] for k in ("base", "extension")
    ):
        raise ResourceWait("reader disk reserve")
    with tempfile.TemporaryDirectory(prefix="reader-", dir=scratch) as directory:
        work = Path(directory)
        for name in ("base", "extension"):
            stage(client, bucket, catalog[name], work / (name + ".parquet"))
        from tape_data_product.features.july_r2_batches import local_batches

        yield from local_batches(
            catalog,
            work / "base.parquet",
            work / "extension.parquet",
            columns=columns,
...
def builder_extension_v1(base_rows, decoded_quote_events, start, *, check=check_yield):
    ...
    """
    movement = C.Movement()
    age = C.MidpointAge(decoded_quote_events, start, check)
...
def recover(client, bucket, ident):
    root = prefix(ident)
    manifest = STORE.read_manifest(client, bucket, root + "/manifest.json")
    if manifest is None:
        return None
    if manifest["identity"] != ident or manifest["state"] != "complete":
        raise ValueError("completion identity mismatch")
    if manifest["verification"]["rows_verified"] != ident["seconds"] or manifest[
        "verification"
    ]["reconstructed_fields"] != list(C.FEATURES):
        raise ValueError("incomplete verification")
    for obj in manifest["artifacts"].values():
        verify_remote(client, bucket, obj)
    catalog = STORE.read_manifest(client, bucket, root + "/catalog.json")
    validate_catalog(catalog)
    if (
        catalog["identity"] != ident
        or catalog["catalog_hash"] != manifest["catalog_hash"]
    ):
        raise ValueError("completion catalog mismatch")
    if catalog["extension"] != manifest["artifacts"]["extension"]:
        raise ValueError("completion extension mismatch")
...
def build_publish(
    ...
    *,
    ...
    ident = identity(payload, seconds, inventory_hash, probe_run)
    root = prefix(ident)
    ...
    if prior is not None:
        for name in ("catalog", "coverage"):
            ...
            if sha(path) != obj["sha256"] or path.stat().st_size != obj["size_bytes"]:
                raise ValueError("recovered control artifact identity mismatch")
        save(
            receipt_path,
            dict(
    ...
    if work.exists() and any(work.iterdir()):
        raise ValueError("incomplete private attempt requires reviewed recovery")
    ...
    for name in (
        ...
        if S._head_or_none(client, bucket, root + "/" + name) is not None:
            raise ValueError(
                "uncommitted remote publication requires reviewed recovery: " + root
            )
    ...
    pf = pq.ParquetFile(work / "base.parquet")
    C_schema = json.loads(pf.schema_arrow.metadata[b"tape_snapshot"])
    from tape_data_product.features.all_feature_month_schema import capabilities

    capabilities(pf.schema_arrow)
    if C_schema["expected_rows"] != base_id["rows"]:
        raise ValueError("base embedded coverage mismatch")
    if "partition_identity" in payload["base"]:
        binding = payload["base"]["partition_identity"]
        prov = C_schema["provenance"]
        if (
            prov["source"] != binding["source"]
            or prov["config_hash"] != digest(binding)
            or prov["halt_policy"] != binding["halt_policy"]
            or C_schema["contract"] != payload["base"]["contract"]
            or C_schema["contract"] != STORE.F.identity()
        ):
            raise ValueError("base embedded provenance mismatch")
    stats = Counter()
    tick = time.monotonic()
    last = [0.0]
    ...
    def check():
        ...
        if time.monotonic() - last[0] > 2:
            progress(phase="compute", events=stats.get("quote_rows_consumed", 0))
    ...
    catalog = dict(
        ...
        base=base_id,
        extension=dict(
            object_key=root + "/extension.parquet",
            sha256=ext["sha256"],
            size_bytes=ext["bytes"],
            rows=seconds,
        ),
        discovery=payload["discovery"],
        conversions=C.field_metadata(),
        columns=C.final_columns(),
        coverage_claim="acquired universe; historical halts and nominal discovery do not establish live availability",
    )
    catalog["catalog_hash"] = digest(catalog)
    save(work / "catalog.json", catalog)
    progress(
        phase="stored_verification", rows=seconds, events=stats["quote_events_reduced"]
    )
    from tape_data_product.features.july_r2_batches import verify, local_batches
    ...
    save(work / "coverage.json", verified.pop("coverage"))
    save(work / "validation.json", verified)
    tick = time.monotonic()
    artifacts = {}
    for name, file in (
        ...
        publisher = (
            S.publish_parquet_immutable
            if name == "extension"
            else S.publish_file_immutable
        )
        artifacts[name] = object_id(
            publisher(client, bucket, work / file, root + "/" + file, upload_workers=1)
        )
    ...
    marker = S.publish_file_immutable(
        client,
        bucket,
        work / "manifest.json",
        root + "/manifest.json",
    ...
    if STORE.read_manifest(client, bucket, root + "/manifest.json") != manifest:
        raise ValueError("completion recovery failed")
    receipt = dict(
        state="published_verified",
        manifest=manifest,
        manifest_key=root + "/manifest.json",
        manifest_object=object_id(marker),
        phase_seconds=phase_times,
        quote_stats=dict(stats),
        rows=seconds,
        staged_bytes=source["quotes"]["size_bytes"] + base_id["size_bytes"],
        uploaded_bytes=sum(x["size_bytes"] for x in artifacts.values())
        + marker["size_bytes"],
        scratch_bytes=sum(p.stat().st_size for p in work.iterdir()),
...
def raw_product_rows_v1(
    ...
    *,
    ...
    start = C.session_start(day)
    age = C.MidpointAge((), start)
    movement = C.Movement()
    model = C.V.FeatureStream()
    ...
    def reader(path, stream, date, batch, statistics):
        for item in C.V._primitive_events(path, stream, date, batch, statistics):
            ...
            if ts < start:
                if (
                    ts >= start - 300 * C.NS
                    and not age.interval_row["halt_interval_active"]
                ):
                    age.current = event
                    age.latest_quote = ts
                    age.mid = event["midpoint"] if event["price_state_valid"] else None
                    age.origin = start if age.mid is not None else None
            elif ts < start + seconds * C.NS:
                age.consume_event(ts, event)

    primitives = C.V.primitives(
        quotes,
        trades,
        day,
        symbol,
    ...
    for position in range(seconds):
        left = start + position * C.NS
        right = left + C.NS
        while index < len(intervals) and intervals[index][1] <= left:
            index += 1
        active = bool(
            index < len(intervals)
            and intervals[index][0] < right
            and intervals[index][1] > left
        )
        if left in breaks and not active and not previous_halt:
            epoch += 1
        age.begin_interval(
            dict(
                interval_end_ns=right,
                continuity_segment_id=epoch,
                halt_interval_active=active,
                primitive_quote_source_file_accepted=True,
            )
        )
        primitive = next(primitives)
        base = STORE.F.enrich(model.push(primitive), primitive)
        extension = (
            {k: base[k] for k in C.KEYS} | movement.push(base) | age.end_interval(base)
        )
    ...
    for _ in primitives:
        raise ValueError("unexpected extra primitive")


def dataset_batches(
    client, bucket, manifest_key, scratch, *, columns=None, batch_size=4096
):
    ...
    if manifest is None or manifest["state"] != "complete":
        raise ValueError("dataset not committed")
    if (manifest["partitions"], manifest["rows"]) != (1022, 58867200):
        raise ValueError("incomplete July dataset")
    expected = manifest["objects"]["catalog.jsonl"]
    verify_remote(client, bucket, expected)
    scratch = Path(scratch)
    scratch.mkdir(parents=True, exist_ok=True)
    if shutil.disk_usage(scratch).free < 3 * GiB + expected["size_bytes"]:
        raise ResourceWait("catalog disk reserve")
    with tempfile.TemporaryDirectory(prefix="catalog-", dir=scratch) as directory:
        path = Path(directory) / "catalog.jsonl"
        client.download_file(
            bucket, expected["object_key"], str(path), Config=S.transfer_config(1)
        )
        if (
            path.stat().st_size != expected["size_bytes"]
            or sha(path) != expected["sha256"]
        ):
            raise ValueError("dataset catalog identity mismatch")
        count = rows = 0
        previous = None
        with path.open("rb") as f:
            while line := f.readline(2 * 1024**2 + 1):
                ...
                if previous is not None and key <= previous:
                    raise ValueError("duplicate or unordered catalog member")
                accepted_codes = manifest.get(
                    "accepted_partition_code_identities", [manifest["code_identity"]]
                )
                if (
                    ident["inventory_hash"] != manifest["inventory_hash"]
                    or ident["code"] not in accepted_codes
                ):
                    raise ValueError("catalog dataset binding mismatch")
                completion = recover(client, bucket, ident)
                if (
                    completion is None
                    or completion["catalog_hash"] != catalog["catalog_hash"]
                ):
                    raise ValueError("catalog member not committed")
                ...
                rows += ident["seconds"]
                previous = key
        if (count, rows) != (manifest["partitions"], manifest["rows"]):
            raise ValueError("dataset catalog reconciliation mismatch")
...
def recover_attempt(
    ...
    *,
    ...
    if not review_reason or not review_reason.strip():
        raise ValueError("recorded diagnosis required")
    work = Path(work)
    ident = identity(payload, seconds, inventory_hash)
    root = prefix(ident)
    attempt = read(work / "attempt.json")
    catalog = read(work / "catalog.json")
    if attempt["identity"] != ident or catalog["identity"] != ident:
        raise ValueError("recovery code/input identity changed")
    validate_catalog(catalog)
    # Verify surviving source/extension bytes and all values/masks again.
    verified = verify_rows(
        local_rows(catalog, work / "base.parquet", work / "extension.parquet"), seconds
    )
    save(work / "coverage.json", verified.pop("coverage"))
    save(work / "validation.json", verified)
    ...
    if recover(client, bucket, ident) != manifest:
        raise ValueError("reviewed recovery did not commit")
    receipt = dict(
        state="reviewed_recovery_verified",
