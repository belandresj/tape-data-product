...
KNOWN_QUOTE_CONDITIONS = frozenset(
    {
        ...
        35, 36, 37, 38, 39, 40, 41, 42, 43, 71, 80, 81, 82, 83, 84, 85,
...
class ProcessedTradeBatch:
    ...
    def to_audit_table(self) -> pa.Table:
        ...
        out = out.append_column("is_odd_lot", pa.array(self.is_odd_lot, type=pa.bool_()))
        out = out.append_column(
        ...
        out = out.append_column("correction_code", pa.array(correction, type=pa.int16()))
        out = out.append_column(
...
def _writer_kwargs(config: PreprocessConfig) -> dict:
    ...
    if (
        config.compression_level is not None
        and config.compression in {"zstd", "gzip", "brotli"}
    ):
...
def _state_change_mask(
    ...
    signature = {
        name: sem[name][-1].item()  # type: ignore[index,union-attr]
        for name in fields
    }
    ...
    *,
    ...
    qa_out = {
        ...
        "compression_ratio_state_rows_over_raw": (
            float(qa["state_rows"] / qa["input_rows"])
            if qa["input_rows"]
            else None
        ),
...
def activity_trade_semantics(
    ...
    *,
    ...
    causal_payload_codes = np.fromiter(
        CAUSAL_TRADE_PAYLOAD_CORRECTIONS, dtype=np.int16
    )
...
def _aggressor_labels(
    ...
    if quote_state.sip_timestamp.size:
        ref_idx = np.searchsorted(
            quote_state.sip_timestamp, trade_sip, side="left"
        ).astype(np.int64) - 1
        has_quote = ref_idx >= 0
...
def _process_trade_table(
    ...
    correction_counts = (
        Counter(
            {
                int(k): int(v)
                for k, v in zip(*np.unique(correction, return_counts=True))
            }
        )
        if correction.size
        else Counter()
        ...
        Counter(
            {
                ...
                for k, v in zip(
                    *np.unique(eligible_confidence, return_counts=True)
                )
            }
        )
...
def _update_trade_qa_aggregate(agg: dict, qa: dict) -> None:
    ...
    if latencies.size:
        ...
        agg["max_latency_ns"] = (
            lmax
            if agg["max_latency_ns"] is None
            else max(agg["max_latency_ns"], lmax)
        )
...
def _finalize_trade_qa(agg: dict, config: PreprocessConfig) -> dict:
    ...
    return {
        ...
        "equal_sip_sequence_inversions": int(
            agg["equal_sip_sequence_inversions"]
        ),
        ...
        "reporting_latency_gate": {
            ...
            "latency_over_threshold_count": int(
                agg["over_latency_threshold_count"]
            ),
            "aggressor_eligible_before_latency_count": int(
            ...
            "exchange": {
                ...
                "negative_latency_count": int(
                    agg["exchange_negative_latency_count"]
                ),
                "latency_over_threshold_count": int(
...
# ---------------------------------------------------------------------------
def _build_qa_manifest(job: Job, config: PreprocessConfig, quote_qa: dict, trade_qa: dict) -> dict:
    return {
...
def _audit_paths(project_root: Path, job: Job) -> AuditPaths:
    ...
    return AuditPaths(
        market_state=audit_root
        / "market_state"
        / job.date
        / f"{job.symbol}.parquet",
        trade_events=audit_root
        / "trade_events"
        / job.date
        / f"{job.symbol}.parquet",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    mode = (
        ...
        else "QA manifest only"
        if args.write_qa
        else "validation only (no outputs)"
    )
    print(
